...
/**
 * The mod_prog submission status updated event.
 *
 * @package    mod_prog
...
/**
 * The mod_prog submission status updated event class.
 *
...
class submission_status_updated extends base {
    ...
    public static function create_from_submission(\prog $prog, \stdClass $submission) {
        $data = array(
            ...
            'relateduserid' => ($prog->get_instance()->teamsubmission) ? null : $submission->userid,
            'other' => array(
    ...
    public static function get_name() {
        return get_string('eventsubmissionstatusupdated', 'mod_prog');
    }
    ...
    protected function get_legacy_logdata() {
        $submission = $this->get_record_snapshot('prog_submission', $this->objectid);
        $user = $this->get_record_snapshot('user', $submission->userid);
        $logmessage = get_string('reverttodraftforstudent', 'prog', array('id' => $user->id, 'fullname' => fullname($user)));
