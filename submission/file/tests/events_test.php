...
class progsubmission_file_events_testcase extends advanced_testcase {
    ...
    /** @var stdClass $prog The assignment object. */
    protected $prog;
    ...
    protected function setUp() {
        ...
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_prog');
        $params['course'] = $this->course->id;
        $instance = $generator->create_instance($params);
        $this->cm = get_coursemodule_from_instance('prog', $instance->id);
        $this->context = context_module::instance($this->cm->id);
        $this->prog = new testable_prog($this->context, $this->cm, $this->course);

        $this->setUser($this->user->id);
        $this->submission = $this->prog->get_user_submission($this->user->id, true);
        ...
        $dummy = (object) array(
            ...
            'itemid' => $this->submission->id,
            'filepath' => '/',
            'filename' => 'myprogmnent.png'
        );
        $this->fi2 = $fs->create_file_from_string($dummy, 'Content of ' . $dummy->filename);
        $this->files = $fs->get_area_files($this->context->id, 'progsubmission_file', PROGSUBMISSION_FILE_FILEAREA,
    ...
    public function test_assessable_uploaded() {
        ...
        $data = new stdClass();
        $plugin = $this->prog->get_submission_plugin_by_type('file');
        ...
        $event = reset($events);
        $this->assertInstanceOf('\progsubmission_file\event\assessable_uploaded', $event);
    ...
    public function test_submission_created() {
        ...
        $event = $events[1];
        $this->assertInstanceOf('\progsubmission_file\event\submission_created', $event);
        $this->assertEquals($this->context->id, $event->contextid);
    ...
    public function test_submission_updated() {
        ...
        // We want to test the last event fired.
        $event = $events[3];
        $this->assertInstanceOf('\progsubmission_file\event\submission_updated', $event);
        $this->assertEquals($this->context->id, $event->contextid);
        $this->assertEquals($this->course->id, $event->courseid);
        $this->assertEquals($this->submission->id, $event->other['submissionid']);
        $this->assertEquals($this->submission->attemptnumber, $event->other['submissionattempt']);
