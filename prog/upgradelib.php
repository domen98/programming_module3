...
/**
 * This file contains the upgrade code to upgrade from mod_assignment to mod_prog
 *
 * @package   mod_prog
 ...
 */
define('PROG_MAX_UPGRADE_TIME_SECS', 300);

/**
 * Class to manage upgrades from mod_assignment to mod_prog
 *
 * @package   mod_prog
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class prog_upgrade_manager {
    ...
    public function upgrade_assignment($oldassignmentid, & $log) {
        ...
        core_php_time_limit::raise(PROG_MAX_UPGRADE_TIME_SECS);
        ...
        if (!has_capability('mod/prog:addinstance', $oldcontext)) {
            $log = get_string('couldnotcreatenewassignmentinstance', 'mod_prog');
            return false;
        }

        // First insert an prog instance to get the id.
        ...
        $data->maxattempts = PROG_UNLIMITED_ATTEMPTS;

        $newassignment = new prog(null, null, null);
        ...
        $newmodule = $DB->get_record('modules', array('name'=>'prog'), '*', MUST_EXIST);
        $newcoursemodule = $this->duplicate_course_module($oldcoursemodule,
                                                          $newmodule->id,
                                                          $newassignment->get_instance()->id);
        if (!$newcoursemodule) {
            $log = get_string('couldnotcreatenewcoursemodule', 'mod_prog');
            return false;
        }
        ...
        try {
            ...
            $newassignment->copy_area_files_for_upgrade($oldcontext->id, 'mod_assignment', 'intro', 0,
                                            $newassignment->get_context()->id, 'mod_prog', 'intro', 0);
            ...
            $DB->set_field('log', 'module', 'prog', $logparams);
            ...
            foreach ($oldsubmissions as $oldsubmission) {
                ...
                $submission->latest = 1;
                $submission->id = $DB->insert_record('prog_submission', $submission);
                if (!$submission->id) {
                    $log .= get_string('couldnotinsertsubmission', 'mod_prog', $submission->userid);
                    $rollback = true;
                }
                foreach ($newassignment->get_submission_plugins() as $plugin) {
                    if ($plugin->can_upgrade($oldassignment->assignmenttype, $oldversion)) {
                ...
                if ($oldsubmission->timemarked) {
                    ...
                    $grade->id = $DB->insert_record('prog_grades', $grade);
                    if (!$grade->id) {
                        $log .= get_string('couldnotinsertgrade', 'mod_prog', $grade->userid);
                        $rollback = true;
                    }
            ...
            // Reassociate grade_items from the old assignment instance to the new prog instance.
            // This includes outcome linked grade_items.
            $params = array('prog', $newassignment->get_instance()->id, 'assignment', $oldassignment->id);
            ...
            $rollback = true;
            $log .= get_string('conversionexception', 'mod_prog', $exception->getMessage());
        }

        if ($rollback) {
            // Roll back the grades changes.
            if ($gradesdone) {
                // Reassociate grade_items from the new prog instance to the old assignment instance.
                $params = array('assignment', $oldassignment->id, 'prog', $newassignment->get_instance()->id);
                $sql = 'UPDATE {grade_items} SET itemmodule = ?, iteminstance = ? WHERE itemmodule = ? AND iteminstance = ?';
                $DB->execute($sql, $params);
            }
     ...
     * prog module alongside the old assignment module.
     *
     * @param stdClass $cm The old course module record
     * @param int $moduleid The id of the new prog module
     * @param int $newinstanceid The id of the new instance of the prog module
