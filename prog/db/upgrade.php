 ...
 * @param int $oldversion The old version of the prog module
 * @return bool
 */
function xmldb_prog_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2012051700) {
        ...
        // Define field to be added to prog.
        ...
        upgrade_mod_savepoint(true, 2012051700, 'prog');
    }
    ...
    if ($oldversion < 2012071800) {
        ...
        // Define field requiresubmissionstatement to be added to prog.
        $table = new xmldb_table('prog');
        ...
        // Assign savepoint reached.
        upgrade_mod_savepoint(true, 2012071800, 'prog');
    }
    ...
    if ($oldversion < 2012081600) {
        ...
        // Assign savepoint reached.
        upgrade_mod_savepoint(true, 2012081600, 'prog');
    }

    // Individual extension dates support.
    if ($oldversion < 2012082100) {

        // Define field cutoffdate to be added to prog.
        $table = new xmldb_table('prog');
        ...
        if ($dbman->field_exists($table, $field)) {
            ...
            $sql = 'UPDATE {prog} SET cutoffdate = duedate WHERE preventlatesubmissions = 1';
            $DB->execute($sql);

            $dbman->drop_field($table, $field);
        }

        // Define field extensionduedate to be added to prog_grades.
        $table = new xmldb_table('prog_grades');
        ...
        // Assign savepoint reached.
        upgrade_mod_savepoint(true, 2012082100, 'prog');
    }
    ...
    if ($oldversion < 2012082300) {
        ...
        $table = new xmldb_table('prog_submission');
        ...
        upgrade_mod_savepoint(true, 2012082300, 'prog');
    }
    if ($oldversion < 2012082400) {

        // Define table prog_user_mapping to be created.
        $table = new xmldb_table('prog_user_mapping');

        // Adding fields to table prog_user_mapping.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('assignment', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        $table->add_field('userid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');

        // Adding keys to table prog_user_mapping.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->add_key('assignment', XMLDB_KEY_FOREIGN, array('assignment'), 'prog', array('id'));
        $table->add_key('user', XMLDB_KEY_FOREIGN, array('userid'), 'user', array('id'));

        // Conditionally launch create table for prog_user_mapping.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Define field blindmarking to be added to prog.
        $table = new xmldb_table('prog');
        ...
        // Define field revealidentities to be added to prog.
        $table = new xmldb_table('prog');
        ...
        upgrade_mod_savepoint(true, 2012082400, 'prog');
    }
    ...
    if ($oldversion < 2013030600) {
        ...
        // Some assignments (upgraded from 2.2 assignment) have duplicate entries in the prog_submission
        // and prog_grades tables for a single user. This needs to be cleaned up before we can add the unique indexes
        // below.
        ...
        if (!$dbman->field_exists($table, $field)) {
            ...
            $sql = 'SELECT assignment, userid, groupid from {prog_submission} ' .
            ...
            foreach ($badrecords as $badrecord) {
                ...
                $duplicates = $DB->get_records('prog_submission', $params, 'timemodified DESC', 'id, timemodified');
                if ($duplicates) {
                    ...
                    $DB->delete_records_select('prog_submission', 'id ' . $sqlids, $sqlidparams);
                }
            }
        ...
        // Same cleanup required for prog_grades
        // Only do this cleanup if the attempt number field has not been added to the table yet.
        $table = new xmldb_table('prog_grades');
        ...
        if (!$dbman->field_exists($table, $field)) {
            ...
            $sql = 'SELECT assignment, userid from {prog_grades} GROUP BY assignment, userid HAVING (count(id) > 1)';
            $badrecords = $DB->get_recordset_sql($sql);

            foreach ($badrecords as $badrecord) {
                $params = array('userid'=>$badrecord->userid,
                                'assignment'=>$badrecord->assignment);
                $duplicates = $DB->get_records('prog_grades', $params, 'timemodified DESC', 'id, timemodified');
                if ($duplicates) {
                    // Take the first (last updated) entry out of the list so it doesn't get deleted.
                    $valid = array_shift($duplicates);
                    $deleteids = array();
                    ...
                    list($sqlids, $sqlidparams) = $DB->get_in_or_equal($deleteids);
                    $DB->delete_records_select('prog_grades', 'id ' . $sqlids, $sqlidparams);
                }
            }
        ...
        // Define table prog_user_flags to be created.
        $table = new xmldb_table('prog_user_flags');

        // Adding fields to table prog_user_flags.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('userid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        $table->add_field('assignment', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        ...
        // Adding keys to table prog_user_flags.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->add_key('userid', XMLDB_KEY_FOREIGN, array('userid'), 'user', array('id'));
        $table->add_key('assignment', XMLDB_KEY_FOREIGN, array('assignment'), 'prog', array('id'));

        // Adding indexes to table prog_user_flags.
        $table->add_index('mailed', XMLDB_INDEX_NOTUNIQUE, array('mailed'));

        // Conditionally launch create table for prog_user_flags.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);

            // Copy the flags from the old table to the new one.
            $sql = 'INSERT INTO {prog_user_flags}
        ...
        // Define index mailed (not unique) to be dropped form prog_grades.
        $table = new xmldb_table('prog_grades');
        ...
        // Define field locked to be dropped from prog_grades.
        $table = new xmldb_table('prog_grades');
        ...
        // Define field mailed to be dropped from prog_grades.
        $table = new xmldb_table('prog_grades');
        ...
        // Define field extensionduedate to be dropped from prog_grades.
        $table = new xmldb_table('prog_grades');
        ...
        // Define field attemptreopenmethod to be added to prog.
        $table = new xmldb_table('prog');
        ...
        // Define field maxattempts to be added to prog.
        $table = new xmldb_table('prog');
        ...
        // Define field attemptnumber to be added to prog_submission.
        $table = new xmldb_table('prog_submission');
        $field = new xmldb_field('attemptnumber', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'groupid');
        ...
        // Define index attemptnumber (not unique) to be added to prog_submission.
        $table = new xmldb_table('prog_submission');
        ...
        // Define field attemptnumber to be added to prog_grades.
        $table = new xmldb_table('prog_grades');
        $field = new xmldb_field('attemptnumber', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'grade');
        ...
        // Define index attemptnumber (not unique) to be added to prog_grades.
        $table = new xmldb_table('prog_grades');
        $index = new xmldb_index('attemptnumber', XMLDB_INDEX_NOTUNIQUE, array('attemptnumber'));
        ...
        // Define index uniqueattemptsubmission (unique) to be added to prog_submission.
        $table = new xmldb_table('prog_submission');
        ...
        // Define index uniqueattemptgrade (unique) to be added to prog_grades.
        $table = new xmldb_table('prog_grades');
        ...
        // Module prog savepoint reached.
        upgrade_mod_savepoint(true, 2013030600, 'prog');
    }
    ...
    if ($oldversion < 2013061101) {
        // Define field markingworkflow to be added to prog.
        $table = new xmldb_table('prog');
        ...
        // Define field markingallocation to be added to prog.
        $field = new xmldb_field('markingallocation', XMLDB_TYPE_INTEGER, '2', null, XMLDB_NOTNULL, null, '0', 'markingworkflow');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Define field workflowstate to be added to prog_grades.
        $table = new xmldb_table('prog_user_flags');
    ...
    if ($oldversion < 2014010801) {
        ...
        // Define field sendstudentnotifications to be added to prog.
        $table = new xmldb_table('prog');
        ...
        // Assign savepoint reached.
        upgrade_mod_savepoint(true, 2014010801, 'prog');
    }
    ...
    if ($oldversion < 2014051201) {
        ...
        // Cleanup bad database records where progid is missing.

        $DB->delete_records('prog_user_mapping', array('assignment'=>0));
        // Assign savepoint reached.
        upgrade_mod_savepoint(true, 2014051201, 'prog');
    }
    if ($oldversion < 2014072400) {
        ...
        // Assign savepoint reached.
        upgrade_mod_savepoint(true, 2014072400, 'prog');
    }
    if ($oldversion < 2014072401) {

         // Define index latestattempt (not unique) to be added to prog_submission.
        $table = new xmldb_table('prog_submission');
    ...
    if ($oldversion < 2014072405) {
        ...
        if ($count == 0) {
            ...
            if ($DB->get_dbfamily() === 'mysql') {
                ...
                $sql = 'UPDATE {prog_submission}
                    INNER JOIN (' . $maxattemptidssql . ') souterouter ON souterouter.id = {prog_submission}.id
                           SET latest = :latest';
                $DB->execute($sql, $params);
            } else {
                $select = 'id IN(' . $maxattemptidssql . ')';
                $DB->set_field_select('prog_submission', 'latest', 1, $select);
            }
        ...
        // Assign savepoint reached.
        upgrade_mod_savepoint(true, 2014072405, 'prog');
    }
    ...
    if ($oldversion < 2014122600) {
        // Delete any entries from the prog_user_flags and prog_user_mapping that are no longer required.
        if ($DB->get_dbfamily() === 'mysql') {
            $sql1 = "DELETE {prog_user_flags}
                       FROM {prog_user_flags}
                  LEFT JOIN {prog}
                         ON {prog_user_flags}.assignment = {prog}.id
                      WHERE {prog}.id IS NULL";

            $sql2 = "DELETE {prog_user_mapping}
                       FROM {prog_user_mapping}
                  LEFT JOIN {prog}
                         ON {prog_user_mapping}.assignment = {prog}.id
                      WHERE {prog}.id IS NULL";
        } else {
            $sql1 = "DELETE FROM {prog_user_flags}
                WHERE NOT EXISTS (
                          SELECT 'x' FROM {prog}
                           WHERE {prog_user_flags}.assignment = {prog}.id)";

            $sql2 = "DELETE FROM {prog_user_mapping}
                WHERE NOT EXISTS (
                          SELECT 'x' FROM {prog}
                           WHERE {prog_user_mapping}.assignment = {prog}.id)";
        }

        $DB->execute($sql1);
        $DB->execute($sql2);

        upgrade_mod_savepoint(true, 2014122600, 'prog');
    }

    if ($oldversion < 2015022300) {

        // Define field preventsubmissionnotingroup to be added to prog.
        $table = new xmldb_table('prog');
        ...
        // Assign savepoint reached.
        upgrade_mod_savepoint(true, 2015022300, 'prog');
    }
