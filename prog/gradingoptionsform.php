 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_prog_grading_options_form extends moodleform {
    /**
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'general', get_string('gradingoptions', 'prog'));
        // Visible elements.
        $options = array(-1=>get_string('all'), 10=>'10', 20=>'20', 50=>'50', 100=>'100');
        $mform->addElement('select', 'perpage', get_string('assignmentsperpage', 'prog'), $options, $dirtyclass);
        $options = array('' => get_string('filternone', 'prog'),
                         PROG_FILTER_NOT_SUBMITTED => get_string('filternotsubmitted', 'prog'),
                         PROG_FILTER_SUBMITTED => get_string('filtersubmitted', 'prog'),
                         PROG_FILTER_REQUIRE_GRADING => get_string('filterrequiregrading', 'prog'));
        if ($instance['submissionsenabled']) {
            $mform->addElement('select', 'filter', get_string('filter', 'prog'), $options, $dirtyclass);
        }
        if (!empty($instance['markingallocationopt'])) {
            $markingfilter = get_string('markerfilter', 'prog');
        ...
        if (!empty($instance['markingworkflowopt'])) {
            $workflowfilter = get_string('workflowfilter', 'prog');
            $mform->addElement('select', 'workflowfilter', $workflowfilter, $instance['markingworkflowopt'], $dirtyclass);
        }
        // Quickgrading.
        if ($instance['showquickgrading']) {
            $mform->addElement('checkbox', 'quickgrading', get_string('quickgrading', 'prog'), '', $dirtyclass);
            $mform->addHelpButton('quickgrading', 'quickgrading', 'prog');
        ...
        // Buttons.
        $this->add_action_buttons(false, get_string('updatetable', 'prog'));
    }
}
