 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_prog_external extends external_api {
    ...
    /**
     * Returns grade information from prog_grades for the requested assignment ids
    ...
    public static function get_grades($assignmentids, $since = 0) {
        ...
        $placeholders['modname'] = 'prog';
        $cms = $DB->get_records_sql($sql, $placeholders);
        foreach ($cms as $cm) {
            try {
                $context = context_module::instance($cm->id);
                self::validate_context($context);
                require_capability('mod/prog:grade', $context);
            } catch (Exception $e) {
        ...
        if (count ($requestedassignmentids) > 0) {
            ...
            $sql = "SELECT ag.id,
                           ...
                           ag.grade,
                           ag.attemptnumber
                      FROM {prog_grades} ag, {prog_submission} s
    ...
    /**
     * Creates an prog_grades external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.4
     */
    private static function prog_grades() {
    ...
    public static function get_grades_returns() {
        return new external_single_structure(
            array(
                'assignments' => new external_multiple_structure(self::prog_grades(), 'list of assignment grade information'),
    ...
    public static function get_assignments($courseids = array(), $capabilities = array()) {
        ...
        foreach ($courses as $id => $course) {
            ...
            if ($modules = get_coursemodules_in_course('prog', $courses[$id]->id, $extrafields)) {
                foreach ($modules as $module) {
                    $context = context_module::instance($module->id);
                    try {
                        self::validate_context($context);
                        require_capability('mod/prog:view', $context);
                    } catch (Exception $e) {
                        $warnings[] = array(
                            'item' => 'module',
                            'itemid' => $module->id,
                    ...
                    $configrecords = $DB->get_recordset('prog_plugin_config', array('assignment' => $module->assignmentid));
                    $configarray = array();
                    ...
                    $prog = new prog($context, null, null);

                    if ($prog->show_intro()) {

                        list($assignment['intro'], $assignment['introformat']) = external_format_text($module->intro,
                            $module->introformat, $context->id, 'mod_prog', PROG_INTROATTACHMENT_FILEAREA, 0);

                        $fs = get_file_storage();
                        if ($files = $fs->get_area_files($context->id, 'mod_prog', PROG_INTROATTACHMENT_FILEAREA,
                            ...
                            foreach ($files as $file) {
                                ...
                                $assignment['introattachments'][] = array(
                                    ...
                                    'fileurl'  => moodle_url::make_webservice_pluginfile_url(
                                        $context->id, 'mod_prog', PROG_INTROATTACHMENT_FILEAREA, 0, '/', $filename)->out(false)
                                );
                            }
                        }
                    }
    ...
    /**
     * Creates an prog_plugin_config external_single_structure
     *
     * @return external_single_structure
     * @since Moodle 2.4
     */
    private static function get_assignments_config_structure() {
        return new external_single_structure(
            array(
                'id' => new external_value(PARAM_INT, 'prog_plugin_config id'),
    ...
    public static function get_submissions($assignmentids, $status = '', $since = 0, $before = 0) {
        ...
        foreach ($progs as $prog) {
            $submissions = array();
            $submissionplugins = $prog->get_submission_plugins();
            $placeholders = array('progid1' => $prog->get_instance()->id,
                                  'progid2' => $prog->get_instance()->id);

            $submissionmaxattempt = 'SELECT mxs.userid, MAX(mxs.attemptnumber) AS maxattempt
                                     FROM {prog_submission} mxs
                                     WHERE mxs.assignment = :progid1 GROUP BY mxs.userid';

            $sql = "SELECT mas.id, mas.assignment,mas.userid,".
                   "mas.timecreated,mas.timemodified,mas.status,mas.groupid,mas.attemptnumber ".
                   "FROM {prog_submission} mas ".
                   "JOIN ( " . $submissionmaxattempt . " ) smx ON mas.userid = smx.userid ".
                   "WHERE mas.assignment = :progid2 AND mas.attemptnumber = smx.maxattempt";
            ...
            if (!empty($submissionrecords)) {
                ...
                foreach ($submissionrecords as $submissionrecord) {
                    ...
                    foreach ($submissionplugins as $submissionplugin) {
                        ...
                        // Subtype is 'progsubmission', type is currently 'file' or 'onlinetext'.
                        ...
                        foreach ($fileareas as $filearea => $name) {
                            ...
                            foreach ($files as $file) {
                                ...
                                $fileurl = file_encode_url($CFG->wwwroot . '/webservice/pluginfile.php', '/' . $prog->get_context()->id .
            ...
            $assignments[] = array(
                'assignmentid' => $prog->get_instance()->id,
    ...
    /**
     * Creates an prog_submissions external_single_structure
     *
     * @return external_single_structure
     * @since Moodle 2.5
     */
    ...
    public static function set_user_flags($assignmentid, $userflags = array()) {
        ...
        $cm = get_coursemodule_from_instance('prog', $params['assignmentid'], 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);
        self::validate_context($context);
        require_capability('mod/prog:grade', $context);
        $prog = new prog($context, null, null);
        ...
        foreach ($params['userflags'] as $userflag) {
            ...
            $record = $prog->get_user_flags($userflag['userid'], false);
            if ($record) {
                ...
                if ($prog->update_user_flags($record)) {
                    $result['id'] = $record->id;
                ...
                $record = $prog->get_user_flags($userflag['userid'], true);
    ...
    /**
     * Returns user flag information from prog_user_flags for the requested assignment ids
     * @param int[] $assignmentids
    ...
    public static function get_user_flags($assignmentids) {
        ...
        // Create the query and populate an array of prog_user_flags records from the recordset results.
        if (count ($requestedassignmentids) > 0) {
            $placeholders = array();
            list($inorequalsql, $placeholders) = $DB->get_in_or_equal($requestedassignmentids, SQL_PARAMS_NAMED);
    ...
    /**
     * Creates an prog_user_flags external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.6
     */
    private static function prog_user_flags() {
        return new external_single_structure(
            array (
                'assignmentid'    => new external_value(PARAM_INT, 'assignment id'),
    ...
    public static function get_user_flags_returns() {
        return new external_single_structure(
            array(
                'assignments' => new external_multiple_structure(self::prog_user_flags(), 'list of prog user flag information'),
                'warnings'      => new external_warnings('item is always \'assignment\'',
                    'when errorcode is 3 then itemid is an assignment id. When errorcode is 1, itemid is a course module id',
    ...
    /**
     * Returns user mapping information from prog_user_mapping for the requested assignment ids
     * @param int[] $assignmentids
    ...
    public static function get_user_mappings($assignmentids) {
        ...
        foreach ($cms as $cm) {
            try {
                $context = context_module::instance($cm->id);
                self::validate_context($context);
                require_capability('mod/prog:revealidentities', $context);
            } catch (Exception $e) {
                $requestedassignmentids = array_diff($requestedassignmentids, array($cm->instance));
                $warning = array();
                $warning['item'] = 'assignment';
        ...
        // Create the query and populate an array of prog_user_mapping records from the recordset results.
        if (count ($requestedassignmentids) > 0) {
            $placeholders = array();
            list($inorequalsql, $placeholders) = $DB->get_in_or_equal($requestedassignmentids, SQL_PARAMS_NAMED);

            $sql = "SELECT aum.id,aum.assignment,aum.userid ".
                   "FROM {prog_user_mapping} aum ".
    ...
    /**
     * Creates an prog_user_mappings external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.6
     */
    private static function prog_user_mappings() {
        return new external_single_structure(
            array (
                'assignmentid'    => new external_value(PARAM_INT, 'assignment id'),
    ...
    public static function get_user_mappings_returns() {
        return new external_single_structure(
            array(
                'assignments' => new external_multiple_structure(self::prog_user_mappings(), 'list of prog user mapping data'),
                'warnings'      => new external_warnings('item is always \'assignment\'',
                    'when errorcode is 3 then itemid is an assignment id. When errorcode is 1, itemid is a course module id',
    ...
    public static function lock_submissions($assignmentid, $userids) {
        ...
        $assignment = new prog($context, $cm, null);
    ...
    public static function save_submission_parameters() {
        global $CFG;
        require_once("$CFG->dirroot/mod/prog/locallib.php");
        $instance = new prog(null, null, null);
    ...
    public static function copy_previous_attempt($assignmentid) {
        ...
        $cm = get_coursemodule_from_instance('prog', $assignmentid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);
        self::validate_context($context);
    ...
    public static function view_grading_table_parameters() {
        return new external_function_parameters(
            array(
                'progid' => new external_value(PARAM_INT, 'prog instance id')
            )
        );
    }
     ...
     *
     * @param int $progid the prog instance id
     * @return array of warnings and status result
     * @since Moodle 3.0
     * @throws moodle_exception
     */
    public static function view_grading_table($progid) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/prog/locallib.php");
        ...
        $prog = $DB->get_record('prog', array('id' => $params['progid']), 'id', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($prog, 'prog');
        ...
        $prog = new prog($context, null, null);
        $prog->require_view_grades();
        \mod_prog\event\grading_table_viewed::create_from_prog($prog)->trigger();
