 ...
 * @return bool
 */
function progsubmission_comments_comment_validate(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if (!$submission = $DB->get_record('prog_submission', array('id'=>$options->itemid))) {
 ...
 * @return array
 */
function progsubmission_comments_comment_permissions(stdClass $options) {
    global $USER, $CFG, $DB;
 ...
 */
function progsubmission_comments_comment_display($comments, $options) {
    global $CFG, $DB, $USER;
    ...
    if ($assignment->is_blind_marking() && !empty($comments)) {
        ...
        $usermappings = array();
        $hiddenuserstr = trim(get_string('hiddenuser', 'prog'));
 ...
 */
function progsubmission_comments_comment_add(stdClass $comment, stdClass $param) {

    global $DB;
    if ($comment->commentarea == 'submission_comments_upgrade') {
        $submissionid = $comment->itemid;
        $submission = $DB->get_record('prog_submission', array('id' => $submissionid));
