...
/**
 * Unit tests for progfeedback_editpdf\comments_quick_list
 *
...
use \progfeedback_editpdf\comments_quick_list;
use \progfeedback_editpdf\document_services;
use \progfeedback_editpdf\page_editor;
use \progfeedback_editpdf\pdf;
use \progfeedback_editpdf\comment;
use \progfeedback_editpdf\annotation;

global $CFG;
require_once($CFG->dirroot . '/mod/prog/tests/base_test.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class progfeedback_editpdf_testcase extends mod_prog_base_testcase {
    ...
    protected function create_prog_and_submit_pdf() {
        global $CFG;
        $prog = $this->create_instance(array('progsubmission_onlinetext_enabled' => 1,
                                               'progsubmission_file_enabled' => 1,
                                               'progsubmission_file_maxfiles' => 1,
                                               'progfeedback_editpdf_enabled' => 1,
                                               'progsubmission_file_maxsizebytes' => 1000000));

        $user = $this->students[0];
        $this->setUser($user);

        // Create a file submission with the test pdf.
        $submission = $prog->get_user_submission($user->id, true);

        $fs = get_file_storage();
        $pdfsubmission = (object) array(
            'contextid' => $prog->get_context()->id,
            'component' => 'progsubmission_file',
            'filearea' => PROGSUBMISSION_FILE_FILEAREA,
            'itemid' => $submission->id,
            'filepath' => '/',
            'filename' => 'submission.pdf'
        );
        $sourcefile = $CFG->dirroot.'/mod/prog/feedback/editpdf/tests/fixtures/submission.pdf';
        $fi = $fs->create_file_from_pathname($pdfsubmission, $sourcefile);

        $data = new stdClass();
        $plugin = $prog->get_submission_plugin_by_type('file');
    ...
    public function test_page_editor() {
        ...
        $prog = $this->create_prog_and_submit_pdf();
        $this->setUser($this->teachers[0]);

        $grade = $prog->get_user_grade($this->students[0]->id, true);
    ...
    public function test_document_services() {
        ...
        $file = document_services::generate_feedback_document($prog->get_instance()->id, $grade->userid, $grade->attemptnumber);
        $this->assertNotEmpty($file);

        $file2 = document_services::get_feedback_document($prog->get_instance()->id, $grade->userid, $grade->attemptnumber);

        $this->assertEquals($file, $file2);

        document_services::delete_feedback_document($prog->get_instance()->id, $grade->userid, $grade->attemptnumber);
        $file3 = document_services::get_feedback_document($prog->get_instance()->id, $grade->userid, $grade->attemptnumber);
