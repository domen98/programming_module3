...
require_once($CFG->dirroot.'/mod/prog/feedback/editpdf/fpdi/fpdi.php');
...
class pdf extends \FPDI {
    ...
    public function get_image($pageno) {
        ...
        if ($generate) {
            ...
            if (!file_exists($imagefile)) {
                $fullerror = '<pre>'.get_string('command', 'progfeedback_editpdf')."\n";
                $fullerror .= $command . "\n\n";
                $fullerror .= get_string('result', 'progfeedback_editpdf')."\n";
                $fullerror .= htmlspecialchars($result) . "\n\n";
                $fullerror .= get_string('output', 'progfeedback_editpdf')."\n";
                $fullerror .= htmlspecialchars(implode("\n",$output)) . '</pre>';
                throw new \moodle_exception('errorgenerateimage', 'progfeedback_editpdf', '', $fullerror);
            }
        }
    ...
    public static function ensure_pdf_compatible(\stored_file $file) {
        ...
        $temparea = \make_temp_directory('progfeedback_editpdf');
    ...
    public static function test_gs_path($generateimage = true) {
        ...
        $testfile = $CFG->dirroot.'/mod/prog/feedback/editpdf/tests/fixtures/testgs.pdf';
        if (!file_exists($testfile)) {
            $ret->status = self::GSPATH_NOTESTFILE;
            return $ret;
        }

        $testimagefolder = \make_temp_directory('progfeedback_editpdf_test');
