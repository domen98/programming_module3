...
/**
 * This file contains the ingest manager for the progfeedback_editpdf plugin
 *
...
class document_services {
    ...
    private static function get_assignment_from_param($assignment) {
        ...
        if (!is_object($assignment)) {
            $cm = \get_coursemodule_from_instance('prog', $assignment, 0, false, MUST_EXIST);
    ...
    public static function generate_combined_pdf_for_attempt($assignment, $userid, $attemptnumber) {
        ...
        if ($files) {
            ...
            $tmpdir = \make_temp_directory('progfeedback_editpdf/combined/' . self::hash($assignment, $userid, $attemptnumber));
        ...
        $record->component = 'progfeedback_editpdf';
    ...
    public static function page_number_for_attempt($assignment, $userid, $attemptnumber, $readonly = false) {
        ...
        if ($readonly) {
            $grade = $assignment->get_user_grade($userid, true, $attemptnumber);
            $fs = get_file_storage();
            $files = $fs->get_directory_files($assignment->get_context()->id, 'progfeedback_editpdf',
        ...
        $tmpdir = \make_temp_directory('progfeedback_editpdf/pagetotal/'
    ...
    public static function generate_page_images_for_attempt($assignment, $userid, $attemptnumber) {
        ...
        $tmpdir = \make_temp_directory('progfeedback_editpdf/pageimages/' . self::hash($assignment, $userid, $attemptnumber));
        $combined = $tmpdir . '/' . self::COMBINED_PDF_FILENAME;
        $file->copy_content_to($combined); // Copy the file.
    ...
    public static function generate_feedback_document($assignment, $userid, $attemptnumber) {
        ...
        $tmpdir = \make_temp_directory('progfeedback_editpdf/final/' . self::hash($assignment, $userid, $attemptnumber));
        $combined = $tmpdir . '/' . self::COMBINED_PDF_FILENAME;
        $file->copy_content_to($combined); // Copy the file.
        ...
        $fs = \get_file_storage();
        $stamptmpdir = \make_temp_directory('progfeedback_editpdf/stamps/' . self::hash($assignment, $userid, $attemptnumber));
        $grade = $assignment->get_user_grade($userid, true, $attemptnumber);
     ...
     *
     * @param int|\prog $assignment The assignment.
