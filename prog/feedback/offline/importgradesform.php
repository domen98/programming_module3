 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class progfeedback_offline_import_grades_form extends moodleform implements renderable {
    ...
    public function definition() {
        ...
        $renderer = $PAGE->get_renderer('prog');
        ...
        $mform->addElement('header', 'importgrades', get_string('importgrades', 'progfeedback_offline'));
        ...
        while ($record = $gradeimporter->next()) {
            ...
            if ($assignment->is_blind_marking()) {
                $userdesc = get_string('hiddenuser', 'prog') . $assignment->get_uniqueid_for_user($user->id);
            }
            ...
            if (!$skip) {
                ...
                $updates[] = get_string('gradeupdate', 'progfeedback_offline',
            ...
            if ($ignoremodified || !$stalemodificationdate) {
                foreach ($record->feedback as $feedback) {
                    ...
                    if ($newvalue != $oldvalue) {
                        $update = true;
                        $updates[] = get_string('feedbackupdate', 'progfeedback_offline',
        ...
        if ($update) {
            ...
            $mform->addElement('html', get_string('nochanges', 'progfeedback_offline'));
        }
        ...
        $mform->addElement('hidden', 'pluginsubtype', 'progfeedback');
