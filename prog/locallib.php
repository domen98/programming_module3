...
define('PROG_SUBMISSION_STATUS_NEW', 'new');
define('PROG_SUBMISSION_STATUS_REOPENED', 'reopened');
define('PROG_SUBMISSION_STATUS_DRAFT', 'draft');
define('PROG_SUBMISSION_STATUS_SUBMITTED', 'submitted');

// Search filters for grading page.
define('PROG_FILTER_SUBMITTED', 'submitted');
define('PROG_FILTER_NOT_SUBMITTED', 'notsubmitted');
define('PROG_FILTER_SINGLE_USER', 'singleuser');
define('PROG_FILTER_REQUIRE_GRADING', 'require_grading');

// Marker filter for grading page.
define('PROG_MARKER_FILTER_NO_MARKER', -1);

// Reopen attempt methods.
define('PROG_ATTEMPT_REOPEN_METHOD_NONE', 'none');
define('PROG_ATTEMPT_REOPEN_METHOD_MANUAL', 'manual');
define('PROG_ATTEMPT_REOPEN_METHOD_UNTILPASS', 'untilpass');

// Special value means allow unlimited attempts.
define('PROG_UNLIMITED_ATTEMPTS', -1);

// Grading states.
define('PROG_GRADING_STATUS_GRADED', 'graded');
define('PROG_GRADING_STATUS_NOT_GRADED', 'notgraded');

// Marking workflow states.
define('PROG_MARKING_WORKFLOW_STATE_NOTMARKED', 'notmarked');
define('PROG_MARKING_WORKFLOW_STATE_INMARKING', 'inmarking');
define('PROG_MARKING_WORKFLOW_STATE_READYFORREVIEW', 'readyforreview');
define('PROG_MARKING_WORKFLOW_STATE_INREVIEW', 'inreview');
define('PROG_MARKING_WORKFLOW_STATE_READYFORRELEASE', 'readyforrelease');
define('PROG_MARKING_WORKFLOW_STATE_RELEASED', 'released');

// Name of file area for intro attachments.
define('PROG_INTROATTACHMENT_FILEAREA', 'introattachment');

require_once($CFG->libdir . '/accesslib.php');
require_once($CFG->libdir . '/formslib.php');
require_once($CFG->dirroot . '/repository/lib.php');
require_once($CFG->dirroot . '/mod/prog/mod_form.php');
require_once($CFG->libdir . '/gradelib.php');
require_once($CFG->dirroot . '/grade/grading/lib.php');
require_once($CFG->dirroot . '/mod/prog/feedbackplugin.php');
...
/**
 * Standard base class for mod_prog (assignment types).
 *
 * @package   mod_prog
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class prog {
    ...
    /** @var stdClass the assignment record that contains the global settings for this prog instance */
    private $instance;

    /** @var stdClass the grade_item record for this prog instance's primary grade item. */
    private $gradeitem;

    /** @var context the context of the course module for this prog instance
     *               (or just the course if we are creating a new one)
     */
    private $context;

    /** @var stdClass the course this prog instance belongs to */
    private $course;

    /** @var stdClass the admin config for all prog instances  */
    private $adminconfig;

    /** @var prog_renderer the custom renderer for this module */
    private $output;

    /** @var cm_info the course module for this prog instance */
    ...
    /**
     * Constructor for the base prog class.
     *
    ...
    public function __construct($coursemodulecontext, $coursemodule, $course) {
        ...
        $this->submissionplugins = $this->load_plugins('progsubmission');
        $this->feedbackplugins = $this->load_plugins('progfeedback');

        // Extra entropy is required for uniqid() to work on cygwin.
        $this->useridlistid = clean_param(uniqid('', true), PARAM_ALPHANUM);

        if (!isset($SESSION->mod_prog_useridlist)) {
            $SESSION->mod_prog_useridlist = [];
        }
    }
     ...
     *
     * @param string $subtype progsubmission | progfeedback
     * @param string $type
     * @return mixed prog_plugin|null
     */
    public function get_plugin_by_type($subtype, $type) {
        $shortsubtype = substr($subtype, strlen('prog'));
     ...
     * @return mixed prog_feedback_plugin|null
     */
    public function get_feedback_plugin_by_type($type) {
        return $this->get_plugin_by_type('progfeedback', $type);
    }
     ...
     *
     * @param string $type - The type of plugin e.g comments
     * @return mixed prog_submission_plugin|null
     */
    public function get_submission_plugin_by_type($type) {
        return $this->get_plugin_by_type('progsubmission', $type);
    }
    ...
    protected function load_plugins($subtype) {
        ...
        foreach ($names as $name => $path) {
            if (file_exists($path . '/locallib.php')) {
                ...
                $shortsubtype = substr($subtype, strlen('prog'));
                $pluginclass = 'prog_' . $shortsubtype . '_' . $name;

                $plugin = new $pluginclass($this, $name);

                if ($plugin instanceof prog_plugin) {
    ...
    public function view($action='') {
        ...
        if ($action == 'redirect') {
            $nextpageurl = new moodle_url('/mod/prog/view.php', $nextpageparams);
            redirect($nextpageurl);
            return;
        } else if ($action == 'savegradingresult') {
            $message = get_string('gradingchangessaved', 'prog');
        ...
        } else if ($action == 'viewpluginprogfeedback') {
            $o .= $this->view_plugin_content('progfeedback');
        } else if ($action == 'viewpluginprogsubmission') {
            $o .= $this->view_plugin_content('progsubmission');
            ...
            $o .= $this->view_error_page(get_string('submitforgrading', 'prog'), $notices);
        } else {
    ...
    public function add_instance(stdClass $formdata, $callplugins) {
        ...
        $update->attemptreopenmethod = PROG_ATTEMPT_REOPEN_METHOD_NONE;
        ...
        $returnid = $DB->insert_record('prog', $update);
        $this->instance = $DB->get_record('prog', array('id'=>$returnid), '*', MUST_EXIST);
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('prog_submission', array('assignment' => $this->get_instance()->id));
        $DB->delete_records('prog_grades', array('assignment' => $this->get_instance()->id));
        $DB->delete_records('prog_plugin_config', array('assignment' => $this->get_instance()->id));
        $DB->delete_records('prog_user_flags', array('assignment' => $this->get_instance()->id));
        $DB->delete_records('prog_user_mapping', array('assignment' => $this->get_instance()->id));
        ...
        // Delete the instance.
        $DB->delete_records('prog', array('id'=>$this->get_instance()->id));
    ...
    public function reset_userdata($data) {
        ...
        $componentstr = get_string('modulenameplural', 'prog');
        $status = array();

        $fs = get_file_storage();
        if (!empty($data->reset_prog_submissions)) {
            // Delete files associated with this assignment.
            foreach ($this->submissionplugins as $plugin) {
                $fileareas = array();
                ...
                if (!$plugin->delete_instance()) {
                    $status[] = array('component'=>$componentstr,
                                      'item'=>get_string('deleteallsubmissions', 'prog'),
            ...
            $progids = $DB->get_records('prog', array('course' => $data->courseid), '', 'id');
            list($sql, $params) = $DB->get_in_or_equal(array_keys($progids));

            $DB->delete_records_select('prog_submission', "assignment $sql", $params);
            $DB->delete_records_select('prog_user_flags', "assignment $sql", $params);
            ...
            if (!empty($data->reset_gradebook_grades)) {
                $DB->delete_records_select('prog_grades', "assignment $sql", $params);
                // Remove all grades from gradebook.
                require_once($CFG->dirroot.'/mod/prog/lib.php');
                prog_reset_gradebook($data->courseid);
                ...
                if ($this->get_instance()->blindmarking && $this->get_instance()->revealidentities) {
                    $DB->set_field('prog', 'revealidentities', 0, array('id' => $this->get_instance()->id));
                }
            }
        }
        // Updating dates - shift may be negative too.
        if ($data->timeshift) {
            shift_course_mod_dates('prog',
     ...
     *
     * @param prog_plugin $plugin The plugin to update
     * @param stdClass $formdata The form data
     * @return bool false if an error occurs
     */
    protected function update_plugin_instance(prog_plugin $plugin, stdClass $formdata) {
    ...
    public function update_gradebook($reset, $coursemoduleid) {
        ...
        require_once($CFG->dirroot.'/mod/prog/lib.php');
        $prog = clone $this->get_instance();
        $prog->cmidnumber = $coursemoduleid;

        // Set prog gradebook feedback plugin status (enabled and visible).
        $prog->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
        ...
        return prog_grade_item_update($prog, $param);
    }
    ...
    public function get_admin_config() {
        ...
        $this->adminconfig = get_config('prog');
        return $this->adminconfig;
    }
    ...
    public function update_calendar($coursemoduleid) {
        ...
        if ($instance->duedate) {
            ...
            $params = array('modulename'=>'prog', 'instance'=>$instance->id);
            ...
            $DB->delete_records('event', array('modulename'=>'prog', 'instance'=>$instance->id));
        }
    }
    ...
    public function update_instance($formdata) {
        ...
        $result = $DB->update_record('prog', $update);
        $this->instance = $DB->get_record('prog', array('id'=>$update->id), '*', MUST_EXIST);
    ...
    protected function save_intro_draft_files($formdata) {
        if (isset($formdata->introattachments)) {
            file_save_draft_area_files($formdata->introattachments, $this->get_context()->id,
                                       'mod_prog', PROG_INTROATTACHMENT_FILEAREA, 0);
        }
    }
     ...
     *
     * @param prog_plugin $plugin The plugin to add the settings from
     ...
     * @return void
     */
    protected function add_plugin_settings(prog_plugin $plugin, MoodleQuickForm $mform, & $pluginsenabled) {
        global $CFG;
    ...
    public function add_all_plugin_settings(MoodleQuickForm $mform) {
        $mform->addElement('header', 'submissiontypes', get_string('submissiontypes', 'prog'));

        $submissionpluginsenabled = array();
        $group = $mform->addGroup(array(), 'submissionplugins', get_string('submissiontypes', 'prog'), array(' '), false);
        foreach ($this->submissionplugins as $plugin) {
        ...
        $mform->addElement('header', 'feedbacktypes', get_string('feedbacktypes', 'prog'));
        $feedbackpluginsenabled = array();
        $group = $mform->addGroup(array(), 'feedbackplugins', get_string('feedbacktypes', 'prog'), array(' '), false);
        foreach ($this->feedbackplugins as $plugin) {
    ...
    protected function get_module_name() {
        ...
        self::$modulename = get_string('modulename', 'prog');
        return self::$modulename;
    }
    ...
    protected function get_module_name_plural() {
        ...
        self::$modulenameplural = get_string('modulenameplural', 'prog');
        return self::$modulenameplural;
    }
    ...
    public function get_instance() {
        ...
        if ($this->get_course_module()) {
            ...
            $this->instance = $DB->get_record('prog', $params, '*', MUST_EXIST);
        }
        if (!$this->instance) {
    ...
    /**
     * Get the primary grade item for this prog instance.
     *
    ...
    protected function count_attachments() {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->get_context()->id, 'mod_prog', PROG_INTROATTACHMENT_FILEAREA,
                        0, 'id', false);
    ...
    public function list_participants($currentgroup, $idsonly) {
        ...
        if (!isset($this->participants[$key])) {
            $users = get_enrolled_users($this->context, 'mod/prog:submit', $currentgroup, 'u.*', null, null, null,
    ...
    public function count_submissions_need_grading() {
        ...
        list($esql, $params) = get_enrolled_sql($this->get_context(), 'mod/prog:submit', $currentgroup, true);

        $params['progid'] = $this->get_instance()->id;
    ...
    public function count_submissions($includenew = false) {
        ...
        if (!$includenew) {
            ...
            $params['status'] = PROG_SUBMISSION_STATUS_NEW;
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $currentgroup = groups_get_activity_group($this->get_course_module(), true);
            list($esql, $enrolparams) = get_enrolled_sql($this->get_context(), 'mod/prog:submit', $currentgroup, true);
    ...
    public function count_submissions_with_status($status) {
        ...
        $params['progid'] = $this->get_instance()->id;
        $params['progid2'] = $this->get_instance()->id;
    ...
    protected function get_grading_userid_list() {
        $filter = get_user_preferences('prog_filter', '');
        $table = new prog_grading_table($this, 0, $filter, 0, false);
    ...
    public static function cron() {
        ...
        $timenow   = time();
        $lastcron = $DB->get_field('modules', 'lastcron', array('name' => 'prog'));
        ...
        $sql = "SELECT g.id as gradeid, a.course, a.name, a.blindmarking, a.revealidentities,
                 ...
                 FROM {prog} a
                 JOIN {prog_grades} g ON g.assignment = a.id
            LEFT JOIN {prog_user_flags} uf ON uf.assignment = a.id AND uf.userid = g.userid
                 JOIN {course_modules} cm ON cm.course = a.course AND cm.instance = a.id
                 JOIN {modules} md ON md.id = cm.module AND md.name = 'prog'
        ...
        $params = array(
            'yesterday' => $yesterday,
            'today' => $timenow,
            'wfreleased' => PROG_MARKING_WORKFLOW_STATE_RELEASED,
        );
        ...
        if (!empty($submissions)) {
            ...
            foreach ($submissions as $submission) {
                ...
                $eventtype = 'prog_notification';
                $updatetime = $submission->lastmodified;
                $modulename = get_string('modulename', 'prog');
                ...
                $flags = $DB->get_record('prog_user_flags', array('userid'=>$user->id, 'assignment'=>$submission->assignment));
                if ($flags) {
                    $flags->mailed = 1;
                    $DB->update_record('prog_user_flags', $flags);
                } else {
                    $flags = new stdClass();
        ...
        foreach ($newlyavailable as $record) {
            $cm = get_coursemodule_from_instance('prog', $record->id, 0, false, MUST_EXIST);
    ...
    public function update_user_flags($flags) {
        ...
        $result = $DB->update_record('prog_user_flags', $flags);
        return $result;
    }
    ...
    public function update_grade($grade, $reopenattempt = false) {
        ...
        $DB->update_record('prog_grades', $grade);
        ...
        if ($this->gradebook_item_update(null, $grade)) {
            \mod_prog\event\submission_graded::create_from_grade($this, $grade)->trigger();
        }
    ...
    protected function view_grant_extension($mform) {
        global $DB, $CFG;
        require_once($CFG->dirroot . '/mod/prog/extensionform.php');
        ...
        $header = new prog_header($this->get_instance(),
                                    $this->get_context(),
                                    $this->show_intro(),
                                    $this->get_course_module()->id,
                                    get_string('grantextension', 'prog'));
        ...
        if (!$mform) {
            $formparams = array($this->get_course_module()->id,
                                ...
                                $this->get_instance(),
                                $data);
            $mform = new mod_prog_extension_form(null, $formparams);
        }
        $o .= $this->get_renderer()->render(new prog_form('extensionform', $mform));
        $o .= $this->view_footer();
        return $o;
    }
        ...
        foreach ($members as $id => $member) {
            ...
            if ($submission && $submission->status == PROG_SUBMISSION_STATUS_SUBMITTED) {
                ...
                if ($this->is_blind_marking()) {
                    $members[$id]->alias = get_string('hiddenuser', 'prog') .
    ...
    public function get_group_submission($userid, $groupid, $create, $attemptnumber=-1) {
        ...
        if ($create) {
            ...
            if ($attemptnumber == -1) {
                ...
                $result = $DB->get_records('prog_submission', $params, 'attemptnumber DESC', 'attemptnumber', 0, 1);
                if ($result) {
            ...
            if ($submission->latest) {
                ...
                $DB->set_field('prog_submission', 'latest', 0, $params);
            }
            $submission->status = PROG_SUBMISSION_STATUS_NEW;
            $sid = $DB->insert_record('prog_submission', $submission);
            return $DB->get_record('prog_submission', array('id' => $sid));
        }
        return false;
    }
    ...
    private function view_course_index() {
        ...
        $strplural = get_string('modulenameplural', 'prog');

        if (!$cms = get_coursemodules_in_course('prog', $course->id, 'm.duedate')) {
        ...
        $courseindexsummary = new prog_course_index_summary($usesections, $strsectionname);

        $timenow = time();

        $currentsection = '';
        foreach ($modinfo->instances['prog'] as $cm) {
            if (!$cm->uservisible) {
                continue;
            }
            ...
            if (has_capability('mod/prog:grade', $context)) {
                $submitted = $assignment->count_submissions_with_status(PROG_SUBMISSION_STATUS_SUBMITTED);

            } else if (has_capability('mod/prog:submit', $context)) {
                $usersubmission = $assignment->get_user_submission($USER->id, false);

                if (!empty($usersubmission->status)) {
                    $submitted = get_string('submissionstatus_' . $usersubmission->status, 'prog');
                } else {
                    $submitted = get_string('submissionstatus_', 'prog');
                }
            }
            $gradinginfo = grade_get_grades($course->id, 'mod', 'prog', $cm->instance, $USER->id);
            ...
            $courseindexsummary->add_prog_info($cm->id, $cm->name, $sectionname, $timedue, $submitted, $grade);
    ...
    protected function view_plugin_content($pluginsubtype) {
        ...
        $plugintype = required_param('plugin', PARAM_TEXT);
        $item = null;
        if ($pluginsubtype == 'progsubmission') {
            ...
            $o .= $this->get_renderer()->render(new prog_header($this->get_instance(),
                                                              $this->get_context(),
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new prog_submission_plugin_submission($plugin,
                                                              $item,
                                                              prog_submission_plugin_submission::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing a submission.
            \mod_prog\event\submission_viewed::create_from_submission($this, $item)->trigger();
            ...
            $o .= $this->get_renderer()->render(new prog_header($this->get_instance(),
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new prog_feedback_plugin_feedback($plugin,
                                                              $item,
                                                              prog_feedback_plugin_feedback::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing feedback.
            \mod_prog\event\feedback_viewed::create_from_grade($this, $item)->trigger();
        }
     ...
     * @param prog_plugin $plugin - The assignment plugin
     */
    public function download_rewrite_pluginfile_urls($text, $user, $plugin) {
        ...
        if ($this->is_blind_marking()) {
            $prefix = $groupname . get_string('participant', 'prog');
    ...
    public function render_editor_content($filearea, $submissionid, $plugintype, $editor, $component) {
        ...
        if ($CFG->enableportfolios && has_capability('mod/prog:exportownsubmission', $this->context)) {
            ...
            $portfolioparams = array('cmid' => $this->get_course_module()->id,
                                     ...
                                     'editor' => $editor,
                                     'area'=>$filearea);
            $button->set_callback_options('prog_portfolio_caller', $portfolioparams, 'mod_prog');
            $fs = get_file_storage();
    ...
    protected function view_savegrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new prog_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('savegradingresult', 'prog')));
        $gradingresult = new prog_gradingmessage(get_string('savegradingresult', 'prog'),
                                                   $message,
                                                   $this->get_course_module()->id);
    ...
    protected function view_quickgrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new prog_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('quickgradingresult', 'prog')));
        $lastpage = optional_param('lastpage', null, PARAM_INT);
        $gradingresult = new prog_gradingmessage(get_string('quickgradingresult', 'prog'),
                                                   $message,
                                                   $this->get_course_module()->id,
                                                   false,
                                                   $lastpage);
    ...
    public function require_view_submission($userid) {
        if (!$this->can_view_submission($userid)) {
            throw new required_capability_exception($this->context, 'mod/prog:viewgrades', 'nopermission', '');
        }
    }
    ...
    public function can_view_grades() {
        // Permissions check.
        if (!has_any_capability(array('mod/prog:viewgrades', 'mod/prog:grade'), $this->context)) {
            return false;
        }
    ...
    public function can_grade() {
        // Permissions check.
        if (!has_capability('mod/prog:grade', $this->context)) {
            return false;
        }
    ...
    protected function download_submissions() {
        ...
        $students = get_enrolled_users($this->context, "mod/prog:submit", null, 'u.*', null, null, null,
                        $this->show_only_active_users());
            ...
            if ((groups_is_member($groupid, $userid) or !$groupmode or !$groupid)) {
                ...
                if ($this->get_instance()->teamsubmission) {
                    ...
                    if ($submissiongroup) {
                        ...
                        $groupname = get_string('defaultteam', 'prog') . '-';
                    }
                } else {
                ...
                if ($this->is_blind_marking()) {
                    $prefix = str_replace('_', ' ', $groupname . get_string('participant', 'prog'));
        ...
        if (count($filesforzipping) == 0) {
            $header = new prog_header($this->get_instance(),
                                        ...
                                        '',
                                        $this->get_course_module()->id,
                                        get_string('downloadall', 'prog'));
            $result .= $this->get_renderer()->render($header);
            $result .= $this->get_renderer()->notification(get_string('nosubmission', 'prog'));
            $url = new moodle_url('/mod/prog/view.php', array('id'=>$this->get_course_module()->id,
                                                                    'action'=>'grading'));
            $result .= $this->get_renderer()->continue_button($url);
            $result .= $this->view_footer();
        } else if ($zipfile = $this->pack_files($filesforzipping)) {
            \mod_prog\event\all_submissions_downloaded::create_from_prog($this)->trigger();
    ...
    public function add_to_log($action = '', $info = '', $url='', $return = false) {
        ...
        if ($return) {
            ...
            debugging('The mod_prog add_to_log() function is now deprecated.', DEBUG_DEVELOPER);
            return $args;
        }
    ...
    public function get_renderer() {
        ...
        $this->output = $PAGE->get_renderer('mod_prog');
        return $this->output;
    }
    ...
    protected function get_submission($submissionid) {
        ...
        return $DB->get_record('prog_submission', $params, '*', MUST_EXIST);
    }
    ...
    public function get_user_flags($userid, $create) {
        ...
        $flags = $DB->get_record('prog_user_flags', $params);
        ...
        if ($create) {
            ...
            $fid = $DB->insert_record('prog_user_flags', $flags);
            $flags->id = $fid;
            return $flags;
        }
        return false;
    ...
    public function get_user_grade($userid, $create, $attemptnumber=-1) {
        ...
        $grades = $DB->get_records('prog_grades', $params, 'attemptnumber DESC', '*', 0, 1);
        ...
        if ($create) {
            ...
            $gid = $DB->insert_record('prog_grades', $grade);
            $grade->id = $gid;
            return $grade;
        }
        return false;
    ...
    protected function get_grade($gradeid) {
        ...
        return $DB->get_record('prog_grades', $params, '*', MUST_EXIST);
    }
    ...
    protected function view_single_grade_page($mform) {
        ...
        require_once($CFG->dirroot . '/mod/prog/gradeform.php');

        // Need submit permission to submit an assignment.
        require_capability('mod/prog:grade', $this->context);

        $header = new prog_header($instance,
                                    $this->get_context(),
                                    false,
                                    $this->get_course_module()->id,
                                    get_string('grading', 'prog'));
        $o .= $this->get_renderer()->render($header);
        ...
        if (!$userid) {
            ...
            if (empty($SESSION->mod_prog_useridlist[$useridlistkey])) {
                $SESSION->mod_prog_useridlist[$useridlistkey] = $this->get_grading_userid_list();
            }
            $useridlist = $SESSION->mod_prog_useridlist[$useridlistkey];
        } else {
            $rownum = 0;
        ...
        if ($user) {
            ...
            $usersummary = new prog_user_summary($user,
                                                   $this->get_course()->id,
                                                   $viewfullnames,
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $submissionstatus = new prog_submission_status($instance->allowsubmissionsfromdate,
                                                             ...
                                                             $this->get_course_module()->id,
                                                             $this->get_course()->id,
                                                             prog_submission_status::GRADER_VIEW,
                                                             $showedit,
                                                             false,
                                                             $viewfullnames,
                                                             $extensionduedate,
        ...
        if ($attemptnumber != -1) {
            ...
            $message = get_string('editingpreviousfeedbackwarning', 'prog', $params);
        ...
        if (!$mform) {
            ...
            $mform = new mod_prog_grade_form(null,
                                               $formparams,
                                               'post',
                                               '',
        ...
        $o .= $this->get_renderer()->render(new prog_form('gradingform', $mform));

        if (count($allsubmissions) > 1 && $attemptnumber == -1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new prog_attempt_history($allsubmissions,
                                                  $allgrades,
                                                  $this->get_submission_plugins(),
        ...
        \mod_prog\event\grading_form_viewed::create_from_user($this, $user)->trigger();
    ...
    protected function view_reveal_identities_confirm() {
        require_capability('mod/prog:revealidentities', $this->get_context());
        ...
        $urlparams = array('id'=>$this->get_course_module()->id,
                           ...
                           'sesskey'=>sesskey());
        $confirmurl = new moodle_url('/mod/prog/view.php', $urlparams);

        $urlparams = array('id'=>$this->get_course_module()->id,
                           'action'=>'grading');
        $cancelurl = new moodle_url('/mod/prog/view.php', $urlparams);

        $o .= $this->get_renderer()->confirm(get_string('revealidentitiesconfirm', 'prog'),
                                             $confirmurl,
                                             $cancelurl);
        $o .= $this->view_footer();

        \mod_prog\event\reveal_identities_confirmation_page_viewed::create_from_prog($this)->trigger();
    ...
    protected function view_return_links() {
        ...
        $url = new moodle_url('/mod/prog/view.php', $params);
    ...
    protected function view_grading_table() {
        ...
        require_once($CFG->dirroot . '/mod/prog/gradingoptionsform.php');
        require_once($CFG->dirroot . '/mod/prog/quickgradingform.php');
        require_once($CFG->dirroot . '/mod/prog/gradingbatchoperationsform.php');
        $o = '';
        ...
        if (has_capability('gradereport/grader:view', $this->get_course_context()) &&
            ...
            $links[$gradebookurl] = get_string('viewgradebook', 'prog');
        }
        if ($this->is_any_submission_plugin_enabled() && $this->count_submissions()) {
            $downloadurl = '/mod/prog/view.php?id=' . $cmid . '&action=downloadall';
            $links[$downloadurl] = get_string('downloadall', 'prog');
        }
        if ($this->is_blind_marking() &&
                has_capability('mod/prog:revealidentities', $this->get_context())) {
            $revealidentitiesurl = '/mod/prog/view.php?id=' . $cmid . '&action=revealidentities';
            $links[$revealidentitiesurl] = get_string('revealidentities', 'prog');
        }
        foreach ($this->get_feedback_plugins() as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                foreach ($plugin->get_grading_actions() as $action => $description) {
                    $url = '/mod/prog/view.php' .
                           '?id=' .  $cmid .
                           '&plugin=' . $plugin->get_type() .
                           '&pluginsubtype=progfeedback' .
        ...
        $gradingactions->set_label(get_string('choosegradingaction', 'prog'));

        $gradingmanager = get_grading_manager($this->get_context(), 'mod_prog', 'submissions');

        $perpage = (int) get_user_preferences('prog_perpage', 10);
        $filter = get_user_preferences('prog_filter', '');
        $markerfilter = get_user_preferences('prog_markerfilter', '');
        $workflowfilter = get_user_preferences('prog_workflowfilter', '');
        $controller = $gradingmanager->get_active_controller();
        $showquickgrading = empty($controller) && $this->can_grade();
        $quickgrading = get_user_preferences('prog_quickgrading', false);
        $showonlyactiveenrolopt = has_capability('moodle/course:viewsuspendedusers', $this->context);

        $markingallocation = $this->get_instance()->markingworkflow &&
            $this->get_instance()->markingallocation &&
            has_capability('mod/prog:manageallocations', $this->context);
        // Get markers to use in drop lists.
        $markingallocationoptions = array();
        if ($markingallocation) {
            list($sort, $params) = users_order_by_sql();
            $markers = get_users_by_capability($this->context, 'mod/prog:grade', '', $sort);
            $markingallocationoptions[''] = get_string('filternone', 'prog');
            $markingallocationoptions[PROG_MARKER_FILTER_NO_MARKER] = get_string('markerfilternomarker', 'prog');
        ...
        if ($markingworkflow) {
            $notmarked = get_string('markingworkflowstatenotmarked', 'prog');
            $markingworkflowoptions[''] = get_string('filternone', 'prog');
            $markingworkflowoptions[PROG_MARKING_WORKFLOW_STATE_NOTMARKED] = $notmarked;
        ...
        $gradingoptionsform = new mod_prog_grading_options_form(null,
        ...
        $gradingbatchoperationsform = new mod_prog_grading_batch_operations_form(null,
                                                                                   $batchformparams,
                                                                                   'post',
                                                                                   '',
                                                                                   $classoptions);
        ...
        if ($showquickgrading && $quickgrading) {
            $gradingtable = new prog_grading_table($this, $perpage, $filter, 0, true);
            ...
            $quickformparams = array('cm'=>$this->get_course_module()->id,
                                     ...
                                     'page' => $page);
            $quickgradingform = new mod_prog_quick_grading_form(null, $quickformparams);

            $o .= $this->get_renderer()->render(new prog_form('quickgradingform', $quickgradingform));
        } else {
            $gradingtable = new prog_grading_table($this, $perpage, $filter, 0, false);
        ...
        if ($this->can_grade()) {
            ...
            $SESSION->mod_prog_useridlist[$this->get_useridlist_key()] = $useridlist;
        }
        ...
        if (count($users) != 0 && $this->can_grade()) {
            ...
            $progform = new prog_form('gradingbatchoperationsform', $gradingbatchoperationsform);
            $o .= $this->get_renderer()->render($progform);
        }
        $progform = new prog_form('gradingoptionsform',
                                      $gradingoptionsform,
                                      'M.mod_prog.init_grading_options');
        $o .= $this->get_renderer()->render($progform);
        return $o;
    }
    ...
    protected function view_grading_page() {
        ...
        \mod_prog\event\grading_table_viewed::create_from_prog($this)->trigger();
    ...
    protected function view_notices($title, $notices) {
        ...
        $url = new moodle_url('/mod/prog/view.php', array('id'=>$this->get_course_module()->id, 'action'=>'view'));
    ...
    public function fullname($user) {
        if ($this->is_blind_marking()) {
            $hasviewblind = has_capability('mod/prog:viewblinddetails', $this->get_context());
            if ($hasviewblind) {
                return fullname($user);
            } else {
                $uniqueid = $this->get_uniqueid_for_user($user->id);
                return get_string('participant', 'prog') . ' ' . $uniqueid;
            }
        } else {
            return fullname($user);
        }
    ...
    protected function view_edit_submission_page($mform, $notices) {
        ...
        $o = '';
        require_once($CFG->dirroot . '/mod/prog/submission_form.php');
        // Need submit permission to submit an assignment.
        ...
        if ($userid == $USER->id) {
            ...
            require_capability('mod/prog:submit', $this->context);
            $title = get_string('editsubmission', 'prog');
        } else {
            ...
            $title = get_string('editsubmissionother', 'prog', $name);
        }

        if (!$this->submissions_open($userid)) {
            $message = array(get_string('submissionsclosed', 'prog'));
        ...
        $data->userid = $userid;
        if (!$mform) {
            $mform = new mod_prog_submission_form(null, array($this, $data));
        }
        ...
        $o .= $this->get_renderer()->render(new prog_form('editsubmissionform', $mform));

        $o .= $this->view_footer();

        \mod_prog\event\submission_form_viewed::create_from_user($this, $user)->trigger();
    ...
    public function can_view_submission($userid) {
        ...
        if ($userid == $USER->id && has_capability('mod/prog:submit', $this->context)) {
            return true;
        }
        return false;
    }
    ...
    protected function process_grading_batch_operation(& $mform) {
        ...
        $mform = new mod_prog_grading_batch_operations_form(null,
                                                              $batchformparams,
                                                              'post',
                                                              '',
                                                              $formclasses);
    ...
    protected function view_batch_set_workflow_state($mform) {
        ...
        require_once($CFG->dirroot . '/mod/prog/batchsetmarkingworkflowstateform.php');
        ...
        foreach ($userlist as $userid) {
            if ($usercount >= 5) {
                $usershtml .= get_string('moreusers', 'prog', count($userlist) - 5);
                break;
            }
            $user = $DB->get_record('user', array('id'=>$userid), '*', MUST_EXIST);

            $usershtml .= $this->get_renderer()->render(new prog_user_summary($user,
                                                                $this->get_course()->id,
        ...
        $mform = new mod_prog_batch_set_marking_workflow_state_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new prog_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingworkflowstate', 'prog'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new prog_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_prog\event\batch_set_workflow_state_viewed::create_from_prog($this)->trigger();
    ...
    public function view_batch_markingallocation($mform) {
        ...
        require_once($CFG->dirroot . '/mod/prog/batchsetallocatedmarkerform.php');
        ...
        list($sort, $params) = users_order_by_sql();
        $markers = get_users_by_capability($this->get_context(), 'mod/prog:grade', '', $sort);
        $markerlist = array();
        foreach ($markers as $marker) {
        ...
        $mform = new mod_prog_batch_set_allocatedmarker_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new prog_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingallocation', 'prog'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new prog_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_prog\event\batch_set_marker_allocation_viewed::create_from_prog($this)->trigger();
    ...
    protected function check_submit_for_grading($mform) {
        ...
        require_once($CFG->dirroot . '/mod/prog/submissionconfirmform.php');
        ...
        if ($mform == null) {
            $mform = new mod_prog_confirm_submission_form(null, array($requiresubmissionstatement,
                                                                        $submissionstatement,
                                                                        $this->get_course_module()->id,
                                                                        $data));
        }
        ...
        $submitforgradingpage = new prog_submit_for_grading_page($notifications,
                                                                   $this->get_course_module()->id,
                                                                   $mform);
        $o .= $this->get_renderer()->render($submitforgradingpage);
        $o .= $this->view_footer();

        \mod_prog\event\submission_confirmation_form_viewed::create_from_prog($this)->trigger();
    ...
    public function view_student_summary($user, $showlinks) {
        ...
        if ($this->can_view_submission($user->id)) {
            ...
            $gradingmanager = get_grading_manager($this->context, 'mod_prog', 'submissions');
            ...
            $submissionstatus = new prog_submission_status($instance->allowsubmissionsfromdate,
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $this->get_course()->id,
                                                              prog_submission_status::STUDENT_VIEW,
                                                              $showedit,
                                                              $showsubmit,
                                                              $viewfullnames,
                                                              $extensionduedate,
                                                              ...
                                                              $instance->preventsubmissionnotingroup,
                                                              $usergroups);
            if (has_capability('mod/prog:submit', $this->get_context(), $user)) {
                $o .= $this->get_renderer()->render($submissionstatus);
            }
            ...
            if ($this->get_instance()->markingworkflow && $gradingstatus != PROG_MARKING_WORKFLOW_STATE_RELEASED) {
                $emptyplugins = true; // Don't show feedback plugins until released either.
            }

            $cangrade = has_capability('mod/prog:grade', $this->get_context());
            ...
            if ((!is_null($gradebookgrade->grade) || !$emptyplugins)
                ...
                $feedbackstatus = new prog_feedback_status($gradefordisplay,
                                                      $gradeddate,
                                                      $grader,
                                                      $this->get_feedback_plugins(),
                                                      $grade,
    ...
    protected function show_submit_button($submission = null, $teamsubmission = null, $userid = null) {
        if ($teamsubmission) {
            if ($teamsubmission->status === PROG_SUBMISSION_STATUS_SUBMITTED) {
                // The assignment submission has been completed.
                return false;
            } else if ($this->submission_empty($teamsubmission)) {
                // There is nothing to submit yet.
                return false;
            } else if ($submission && $submission->status === PROG_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if ($submission->status === PROG_SUBMISSION_STATUS_SUBMITTED) {
                // The assignment submission has been completed.
                return false;
    ...
    protected function get_all_grades($userid) {
        ...
        $grades = $DB->get_records('prog_grades', $params, 'attemptnumber ASC');
    ...
    protected function get_all_submissions($userid) {
        ...
        $submissions = $DB->get_records('prog_submission', $params, 'attemptnumber ASC');
    ...
    protected function view_submission_page() {
        ...
        if ($this->has_visible_attachments()) {
            $postfix = $this->render_area_files('mod_prog', PROG_INTROATTACHMENT_FILEAREA, 0);
        }
        $o .= $this->get_renderer()->render(new prog_header($instance,
                                                      $this->get_context(),
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      '', '', $postfix));
        ...
        foreach ($plugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                $o .= $this->get_renderer()->render(new prog_plugin_header($plugin));
            }
        }

        if ($this->can_view_grades()) {
            $draft = PROG_SUBMISSION_STATUS_DRAFT;
            $submitted = PROG_SUBMISSION_STATUS_SUBMITTED;

            // Group selector will only be displayed if necessary.
            $currenturl = new moodle_url('/mod/prog/view.php', array('id' => $this->get_course_module()->id));
            ...
            if ($instance->teamsubmission) {
                ...
                $summary = new prog_grading_summary($this->count_teams($activitygroup),
                ...
                $summary = new prog_grading_summary($countparticipants,
                                                      $instance->submissiondrafts,
                                                      $this->count_submissions_with_status($draft),
                                                      $this->is_any_submission_plugin_enabled(),
                                                      $this->count_submissions_with_status($submitted),
        ...
        \mod_prog\event\submission_status_viewed::create_from_prog($this)->trigger();
    ...
    protected function gradebook_item_update($submission=null, $grade=null) {
        ...
        if ($this->get_instance()->markingworkflow && !empty($grade) &&
                $this->get_grading_status($grade->userid) != PROG_MARKING_WORKFLOW_STATE_RELEASED) {
        ...
        $prog = clone $this->get_instance();
        $prog->cmidnumber = $this->get_course_module()->idnumber;
        // Set prog gradebook feedback plugin status (enabled and visible).
        $prog->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
        return prog_grade_item_update($prog, $gradebookgrade) == GRADE_UPDATE_OK;
    }
    ...
    protected function update_team_submission(stdClass $submission, $userid, $updatetime) {
        ...
        $anysubmitted = false;
        $result = true;
        if ($submission->status != PROG_SUBMISSION_STATUS_REOPENED) {
            foreach ($team as $member) {
                ...
                if (!$membersubmission || $membersubmission->status != PROG_SUBMISSION_STATUS_SUBMITTED
            ...
            if ($this->get_instance()->requireallteammemberssubmit) {
                if ($allsubmitted) {
                    $submission->status = PROG_SUBMISSION_STATUS_SUBMITTED;
                } else {
                    $submission->status = PROG_SUBMISSION_STATUS_DRAFT;
                }
                $result = $DB->update_record('prog_submission', $submission);
            } else {
                if ($anysubmitted) {
                    $submission->status = PROG_SUBMISSION_STATUS_SUBMITTED;
                } else {
            ...
            foreach ($team as $member) {
                ...
                $membersubmission->status = PROG_SUBMISSION_STATUS_REOPENED;
                $result = $DB->update_record('prog_submission', $membersubmission) && $result;
            }
            $result = $DB->update_record('prog_submission', $submission) && $result;
        }
    ...
    public function submissions_open($userid = 0,
        ...
        if ($submission) {
            ...
            if ($this->get_instance()->submissiondrafts && $submission->status == PROG_SUBMISSION_STATUS_SUBMITTED) {
    ...
    public function render_area_files($component, $area, $submissionid) {
        ...
        return $this->get_renderer()->prog_files($this->context, $submissionid, $area, $component);
    ...
    public function can_edit_submission($userid, $graderid = 0) {
        ...
        if ($userid == $graderid &&
                ...
                has_capability('mod/prog:submit', $this->context, $graderid)) {
            // User can edit their own submission.
            return true;
        }

        if (!has_capability('mod/prog:editothersubmission', $this->context, $graderid)) {
            return false;
        }
    ...
    protected function get_graders($userid) {
        ...
        $potentialgraders = get_enrolled_users($this->context, "mod/prog:grade", null, 'u.*', null, null, null, true);
    ...
    protected function get_notifiable_users($userid) {
        ...
        $potentialusers = get_enrolled_users($this->context, "mod/prog:receivegradernotifications",
    ...
    protected static function format_notification_message_text($messagetype,
        ...
        $posttext .= '---------------------------------------------------------------------' . "\n";
        $posttext .= get_string($messagetype . 'text', 'prog', $info)."\n";
        $posttext .= "\n---------------------------------------------------------------------\n";
        return $posttext;
    }
    ...
    protected static function format_notification_message_html($messagetype,
        ...
        $posthtml  = '<p><font face="sans-serif">' .
                     ...
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/prog/index.php?id=' . $course->id . '">' .
                     $modulename .
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/prog/view.php?id=' . $coursemodule->id . '">' .
                     format_string($assignmentname, true, $formatparams) .
                     '</a></font></p>';
        $posthtml .= '<hr /><font face="sans-serif">';
        $posthtml .= '<p>' . get_string($messagetype . 'html', 'prog', $info) . '</p>';
        $posthtml .= '</font><hr />';
        return $posthtml;
    }
    ...
    public static function send_assignment_notification($userfrom,
        ...
        if ($blindmarking) {
            ...
            $info->username = get_string('participant', 'prog') . ' ' . $uniqueidforuser;
            $userfrom->firstname = get_string('participant', 'prog');
        ...
        $info->url = $CFG->wwwroot.'/mod/prog/view.php?id='.$coursemodule->id;
        $info->timeupdated = userdate($updatetime, get_string('strftimerecentfull'));

        $postsubject = get_string($messagetype . 'small', 'prog', $info);
    ...
    public function submit_for_grading($data, $notices) {
        ...
        if (!$this->submissions_open($userid)) {
            $notices[] = get_string('submissionsclosed', 'prog');
            return false;
        }
        ...
        if ($submission->status != PROG_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if (!empty($data->submissionstatement) && $USER->id == $userid) {
                \mod_prog\event\statement_accepted::create_from_submission($this, $submission)->trigger();
            }
            $this->notify_graders($submission);
            $this->notify_student_submission_receipt($submission);

            \mod_prog\event\assessable_submitted::create_from_submission($this, $submission, false)->trigger();
    ...
    public function save_user_extension($userid, $extensionduedate) {
        ...
        // Need submit permission to submit an assignment.
        require_capability('mod/prog:grantextension', $this->context);

        if (!is_enrolled($this->get_course_context(), $userid)) {
            return false;
        }
        if (!has_capability('mod/prog:submit', $this->context, $userid)) {
            return false;
        }
        ...
        if ($result) {
            \mod_prog\event\extension_granted::create_from_prog($this, $userid)->trigger();
        }
        return $result;
    }
    ...
    protected function process_save_extension(& $mform) {
        ...
        $mform = new mod_prog_extension_form(null, array($this->get_course_module()->id,
                                                           $userid,
                                                           $batchusers,
                                                           $this->get_instance(),
                                                           null));
    ...
    protected function process_save_quick_grades() {
        ...
        $controller = $gradingmanager->get_active_controller();
        if (!empty($controller)) {
            return get_string('errorquickgradingvsadvancedgrading', 'prog');
        }
        ...
        $sql = 'SELECT u.id as userid, g.grade as grade, g.timemodified as lastmodified, uf.workflowstate, uf.allocatedmarker
                ...
                LEFT JOIN {prog_grades} g ON
                    u.id = g.userid AND
                    g.assignment = :progid2 AND
                    g.attemptnumber = gmx.maxattempt
                LEFT JOIN {prog_user_flags} uf ON uf.assignment = g.assignment AND uf.userid = g.userid
                WHERE u.id ' . $userids;
                ...
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if ($plugin->is_quickgrading_modified($modified->userid, $grade)) {
                        if ((int)$current->lastmodified > (int)$modified->lastmodified) {
                            return get_string('errorrecordmodified', 'prog');
                        } else {
                            $modifiedusers[$modified->userid] = $modified;
                            continue;
                        }
        ...
        foreach ($modifiedusers as $userid => $modified) {
            ...
            foreach ($this->feedbackplugins as $plugin) {
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if (('progfeedback_' . $plugin->get_type()) == $gradebookplugin) {
            ...
            if ($workflowstatemodified || $allocatedmarkermodified) {
                if ($this->update_user_flags($flags) && $workflowstatemodified) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_prog\event\workflow_state_updated::create_from_user($this, $user, $flags->workflowstate)->trigger();
                }
            }
            ...
            if ($CFG->enableoutcomes) {
                ...
                if (count($data) > 0) {
                    grade_update_outcomes('mod/prog',
                                          $this->course->id,
                                          'mod',
                                          'prog',
                                          $this->get_instance()->id,
        ...
        return get_string('quickgradingchangessaved', 'prog');
    }
    ...
    public function reveal_identities() {
        ...
        require_capability('mod/prog:revealidentities', $this->context);
        ...
        $adminconfig = $this->get_admin_config();
        $gradebookplugin = $adminconfig->feedback_plugin_for_gradebook;
        $gradebookplugin = str_replace('progfeedback_', '', $gradebookplugin);
        $grades = $DB->get_records('prog_grades', array('assignment'=>$this->get_instance()->id));
        ...
        \mod_prog\event\identities_revealed::create_from_prog($this)->trigger();
    }
    ...
    protected function process_save_grading_options() {
        ...
        $mform = new mod_prog_grading_options_form(null, $gradingoptionsparams);
        if ($formdata = $mform->get_data()) {
            set_user_preference('prog_perpage', $formdata->perpage);
            if (isset($formdata->filter)) {
                set_user_preference('prog_filter', $formdata->filter);
            }
            if (isset($formdata->markerfilter)) {
                set_user_preference('prog_markerfilter', $formdata->markerfilter);
            }
            if (isset($formdata->workflowfilter)) {
                set_user_preference('prog_workflowfilter', $formdata->workflowfilter);
            }
            if ($showquickgrading) {
                set_user_preference('prog_quickgrading', isset($formdata->quickgrading));
            }
    ...
    public function format_grade_for_log(stdClass $grade) {
        ...
        $info = get_string('gradestudent', 'prog', array('id'=>$user->id, 'fullname'=>fullname($user)));
        if ($grade->grade != '') {
            $info .= get_string('grade') . ': ' . $this->display_grade($grade->grade, false) . '. ';
        } else {
            $info .= get_string('nograde', 'prog');
        }
        return $info;
    }
    ...
    public function format_submission_for_log(stdClass $submission) {
        ...
        if ($submission->userid) {
            ...
            if ($group) {
                ...
                $name = get_string('defaultteam', 'prog');
            }
        }
        $status = get_string('submissionstatus_' . $submission->status, 'prog');
        $params = array('id'=>$submission->userid, 'fullname'=>$name, 'status'=>$status);
        $info .= get_string('submissionlog', 'prog', $params) . ' <br>';
    ...
    public function copy_previous_attempt(&$notices) {
        ...
        if (!$submission || $submission->status != PROG_SUBMISSION_STATUS_REOPENED) {
            $notices[] = get_string('submissionnotcopiedinvalidstatus', 'prog');
            return false;
        }
        $flags = $this->get_user_flags($USER->id, false);
        ...
        if ($flags && $flags->locked) {
            $notices[] = get_string('submissionslocked', 'prog');
            return false;
        }
        ...
        \mod_prog\event\submission_duplicated::create_from_submission($this, $submission)->trigger();
        ...
        if (!$instance->submissiondrafts) {
            ...
            \mod_prog\event\assessable_submitted::create_from_submission($this, $submission, true)->trigger();
        }
        return true;
    }
    ...
    public function save_submission(stdClass $data, & $notices) {
        ...
        // Get the flags to check if it is locked.
        if ($flags && $flags->locked) {
            print_error('submissionslocked', 'prog');
            return true;
        }
        ...
        if ($pluginerror || $allempty) {
            if ($allempty) {
                $notices[] = get_string('submissionempty', 'mod_prog');
            }
            return false;
        }
    ...
    protected function process_save_submission(&$mform, &$notices) {
        ...
        require_sesskey();
        if (!$this->submissions_open($userid)) {
            $notices[] = get_string('duedatereached', 'prog');
            return false;
        }
        $instance = $this->get_instance();
    ...
    public function add_grade_form_elements(MoodleQuickForm $mform, stdClass $data, $params) {
        ...
        if ($gradinginstance) {
            ...
            if ($this->get_instance()->grade > 0) {
                $name = get_string('gradeoutof', 'prog', $this->get_instance()->grade);
                if (!$gradingdisabled) {
                    $gradingelement = $mform->addElement('text', 'grade', $name);
                    $mform->addHelpButton('grade', 'gradeoutofhelp', 'prog');
                    $mform->setType('grade', PARAM_RAW);
                } else {
                    $mform->addElement('hidden', 'grade', $name);
                    $mform->hardFreeze('grade');
                    $mform->setType('grade', PARAM_RAW);
                    $strgradelocked = get_string('gradelocked', 'prog');
                    $mform->addElement('static', 'gradedisabled', $name, $strgradelocked);
                    $mform->addHelpButton('gradedisabled', 'gradeoutofhelp', 'prog');
                }
            } else {
        ...
        if ($this->get_instance()->markingworkflow) {
            ...
            $options = array('' => get_string('markingworkflowstatenotmarked', 'prog')) + $states;
            $mform->addElement('select', 'workflowstate', get_string('markingworkflowstate', 'prog'), $options);
            $mform->addHelpButton('workflowstate', 'markingworkflowstate', 'prog');
        }
        ...
        if ($this->get_instance()->markingworkflow &&
            ...
            list($sort, $params) = users_order_by_sql();
            $markers = get_users_by_capability($this->context, 'mod/prog:grade', '', $sort);
            $markerlist = array('' =>  get_string('choosemarker', 'prog'));
            foreach ($markers as $marker) {
                $markerlist[$marker->id] = fullname($marker);
            }
            $mform->addElement('select', 'allocatedmarker', get_string('allocatedmarker', 'prog'), $markerlist);
            $mform->addHelpButton('allocatedmarker', 'allocatedmarker', 'prog');
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', PROG_MARKING_WORKFLOW_STATE_READYFORREVIEW);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', PROG_MARKING_WORKFLOW_STATE_INREVIEW);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', PROG_MARKING_WORKFLOW_STATE_READYFORRELEASE);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', PROG_MARKING_WORKFLOW_STATE_RELEASED);
        }

        $mform->addElement('static', 'currentgrade', get_string('currentgrade', 'prog'), $gradestring);

        if (count($useridlist) > 1) {
            $strparams = array('current'=>$rownum+1, 'total'=>count($useridlist));
            $name = get_string('outof', 'prog', $strparams);
            $mform->addElement('static', 'gradingstudent', get_string('gradingstudent', 'prog'), $name);
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            $mform->addElement('header', 'groupsubmissionsettings', get_string('groupsubmissionsettings', 'prog'));
            $mform->addElement('selectyesno', 'applytoall', get_string('applytoteam', 'prog'));
            $mform->setDefault('applytoall', 1);
        }

        // Do not show if we are editing a previous attempt.
        if ($attemptnumber == -1 && $this->get_instance()->attemptreopenmethod != PROG_ATTEMPT_REOPEN_METHOD_NONE) {
            $mform->addElement('header', 'attemptsettings', get_string('attemptsettings', 'prog'));
            $attemptreopenmethod = get_string('attemptreopenmethod_' . $this->get_instance()->attemptreopenmethod, 'prog');
            $mform->addElement('static', 'attemptreopenmethod', get_string('attemptreopenmethod', 'prog'), $attemptreopenmethod);
            ...
            if ($maxattempts == PROG_UNLIMITED_ATTEMPTS) {
                $maxattempts = get_string('unlimitedattempts', 'prog');
            }
            $mform->addelement('static', 'maxattemptslabel', get_string('maxattempts', 'prog'), $maxattempts);
            $mform->addelement('static', 'attemptnumberlabel', get_string('attemptnumber', 'prog'), $attemptnumber + 1);

            $ismanual = $this->get_instance()->attemptreopenmethod == PROG_ATTEMPT_REOPEN_METHOD_MANUAL;
            $issubmission = !empty($submission);
            $isunlimited = $this->get_instance()->maxattempts == PROG_UNLIMITED_ATTEMPTS;
            ...
            if ($ismanual && (!$issubmission || $isunlimited || $islessthanmaxattempts)) {
                $mform->addElement('selectyesno', 'addattempt', get_string('addattempt', 'prog'));
                $mform->setDefault('addattempt', 0);
            }
        }
        $mform->addElement('selectyesno', 'sendstudentnotifications', get_string('sendstudentnotifications', 'prog'));
        ...
        if (!$cm->uservisible) {
            ...
            $mform->setDefault('sendstudentnotifications', 0);
            $mform->disabledIf('sendstudentnotifications', 'workflowstate', 'neq', PROG_MARKING_WORKFLOW_STATE_RELEASED);
        } else {
        ...
        $buttonarray=array();
        $name = get_string('savechanges', 'prog');
        $buttonarray[] = $mform->createElement('submit', 'savegrade', $name);
        if (!$last) {
            $name = get_string('savenext', 'prog');
        ...
        if ($rownum > 0) {
            $name = get_string('previous', 'prog');
            $buttonarray[] = $mform->createElement('submit', 'nosaveandprevious', $name);
        }

        if (!$last) {
            $name = get_string('nosavebutnext', 'prog');
    ...
    public function revert_to_draft($userid) {
        ...
        \mod_prog\event\submission_status_updated::create_from_submission($this, $submission)->trigger();
        return true;
    }
    ...
    public function lock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_prog\event\submission_locked::create_from_user($this, $user)->trigger();
        return true;
    }
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                ...
                if ($formdata->sendstudentnotifications && $cm->uservisible &&
                        $state == PROG_MARKING_WORKFLOW_STATE_RELEASED) {
                    $flags->mailed = 0;
                }

                $gradingdisabled = $this->grading_disabled($userid);

                // Will not apply update if user does not have permission to prog this workflow state.
                if (!$gradingdisabled && $this->update_user_flags($flags)) {
                    if ($state == PROG_MARKING_WORKFLOW_STATE_RELEASED) {
                        // Update Gradebook.
                        $prog = clone $this->get_instance();
                        $prog->cmidnumber = $this->get_course_module()->idnumber;
                        // Set prog gradebook feedback plugin status.
                        $prog->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
                        prog_update_grades($prog, $userid);
                    }

                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_prog\event\workflow_state_updated::create_from_user($this, $user, $state)->trigger();
                }
            }
        }
    }
    ...
    protected function process_set_batch_marking_allocation() {
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                $flags = $this->get_user_flags($userid, true);
                if ($flags->workflowstate == PROG_MARKING_WORKFLOW_STATE_READYFORREVIEW ||
                    $flags->workflowstate == PROG_MARKING_WORKFLOW_STATE_INREVIEW ||
                    $flags->workflowstate == PROG_MARKING_WORKFLOW_STATE_READYFORRELEASE ||
                    $flags->workflowstate == PROG_MARKING_WORKFLOW_STATE_RELEASED) {
                ...
                if ($this->update_user_flags($flags)) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_prog\event\marker_updated::create_from_marker($this, $user, $marker)->trigger();
                }
            }
        }
    }
    ...
    public function unlock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_prog\event\submission_unlocked::create_from_user($this, $user)->trigger();
        return true;
    }
        ...
        if (!$gradingdisabled) {
            ...
            if (isset($formdata->workflowstate) || isset($formdata->allocatedmarker)) {
                ...
                if ($this->update_user_flags($flags) &&
                    ...
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_prog\event\workflow_state_updated::create_from_user($this, $user, $formdata->workflowstate)->trigger();
                }
            }
        }
        $grade->grader= $USER->id;
    ...
    protected function reopen_submission_if_required($userid, $submission, $addattempt) {
        ...
        $maxattemptsreached = !empty($submission) &&
                              ...
                              $instance->maxattempts != PROG_UNLIMITED_ATTEMPTS;
        $shouldreopen = false;
        if ($instance->attemptreopenmethod == PROG_ATTEMPT_REOPEN_METHOD_UNTILPASS) {
        ...
        if ($instance->attemptreopenmethod == PROG_ATTEMPT_REOPEN_METHOD_MANUAL &&
                !empty($addattempt)) {
            $shouldreopen = true;
        }
    ...
    protected function process_save_grade(&$mform) {
        ...
        $userid = optional_param('userid', 0, PARAM_INT);
        if (!$userid) {
            if (empty($SESSION->mod_prog_useridlist[$this->get_useridlist_key($useridlistid)])) {
                ...
                $url = new moodle_url('/mod/prog/view.php', array('id' => $this->get_course_module()->id));
                throw new moodle_exception('useridlistnotcached', 'mod_prog', $url);
            }
            $useridlist = $SESSION->mod_prog_useridlist[$this->get_useridlist_key($useridlistid)];
        } else {
            $useridlist = array($userid);
            $rownum = 0;
        }
    ...
    public static function can_upgrade_assignment($type, $version) {
        $assignment = new prog(null, null, null);
    ...
    protected function add_attempt($userid) {
        ...
        if ($this->get_instance()->attemptreopenmethod == PROG_ATTEMPT_REOPEN_METHOD_NONE) {
            return false;
        }
        ...
        if ($this->get_instance()->maxattempts != PROG_UNLIMITED_ATTEMPTS &&
        ...
        $newsubmission->status = PROG_SUBMISSION_STATUS_REOPENED;
    ...
    public function get_user_grades_for_gradebook($userid) {
        ...
        foreach ($this->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                if (('progfeedback_' . $plugin->get_type()) == $gradebookpluginname) {
        ...
        $params = array('progid1'=>$assignmentid,
                        'progid2'=>$assignmentid,
                        'userid'=>$userid);
        $graderesults = $DB->get_recordset_sql('SELECT
                                                ...
                                                LEFT JOIN {prog_submission} s
                                                    ON u.id = s.userid and s.assignment = :progid1 AND
                                                    s.latest = 1
                                                JOIN {prog_grades} g
                                                    ON u.id = g.userid and g.assignment = :progid2 AND
                                                    g.attemptnumber = s.attemptnumber' .
                                                $where, $params);
    ...
    /**
     * Foreach participant in the course - prog them a random id.
     *
     * @param int $progid The progid to lookup
     */
    public static function allocate_unique_ids($progid) {
        global $DB;

        $cm = get_coursemodule_from_instance('prog', $progid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);

        $currentgroup = groups_get_activity_group($cm, true);
        $users = get_enrolled_users($context, "mod/prog:submit", $currentgroup, 'u.id');

        // Shuffle the users.
        shuffle($users);

        foreach ($users as $user) {
            $record = $DB->get_record('prog_user_mapping',
                                      array('assignment'=>$progid, 'userid'=>$user->id),
                                     'id');
            if (!$record) {
                $record = new stdClass();
                $record->assignment = $progid;
                $record->userid = $user->id;
                $DB->insert_record('prog_user_mapping', $record);
            }
        }
    }
     ...
     *
     * @param int $progid The assignment id
     * @param int $userid The userid to lookup
     * @return int The unique id
     */
    public static function get_uniqueid_for_user_static($progid, $userid) {
        global $DB;

        // Search for a record.
        $params = array('assignment'=>$progid, 'userid'=>$userid);
        if ($record = $DB->get_record('prog_user_mapping', $params, 'id')) {
            return $record->id;
        }
        ...
        self::allocate_unique_ids($progid);
        ...
        return $DB->insert_record('prog_user_mapping', $record);
    }
     ...
     *
     * @param int $progid The id of the assignment this user mapping is in
     * @param int $uniqueid The uniqueid to lookup
     * @return int The user id or false if they don't exist
     */
    public static function get_user_id_for_uniqueid_static($progid, $uniqueid) {
        global $DB;

        // Search for a record.
        if ($record = $DB->get_record('prog_user_mapping',
                                      array('assignment'=>$progid, 'id'=>$uniqueid),
                                      'userid',
                                      IGNORE_MISSING)) {
    ...
    public function get_marking_workflow_states_for_current_user() {
        ...
        $states = array();
        if (has_capability('mod/prog:grade', $this->context)) {
            $states[PROG_MARKING_WORKFLOW_STATE_INMARKING] = get_string('markingworkflowstateinmarking', 'prog');
            $states[PROG_MARKING_WORKFLOW_STATE_READYFORREVIEW] = get_string('markingworkflowstatereadyforreview', 'prog');
        }
        if (has_any_capability(array('mod/prog:reviewgrades',
                                     'mod/prog:managegrades'), $this->context)) {
            $states[PROG_MARKING_WORKFLOW_STATE_INREVIEW] = get_string('markingworkflowstateinreview', 'prog');
            $states[PROG_MARKING_WORKFLOW_STATE_READYFORRELEASE] = get_string('markingworkflowstatereadyforrelease', 'prog');
        }
        if (has_any_capability(array('mod/prog:releasegrades',
                                     'mod/prog:managegrades'), $this->context)) {
            $states[PROG_MARKING_WORKFLOW_STATE_RELEASED] = get_string('markingworkflowstatereleased', 'prog');
        }
    ...
    public function get_grading_status($userid) {
        if ($this->get_instance()->markingworkflow) {
            ...
            return PROG_MARKING_WORKFLOW_STATE_NOTMARKED;
        } else {
            $attemptnumber = optional_param('attemptnumber', -1, PARAM_INT);
            $grade = $this->get_user_grade($userid, false, $attemptnumber);

            if (!empty($grade) && $grade->grade !== null && $grade->grade >= 0) {
                return PROG_GRADING_STATUS_GRADED;
            } else {
                return PROG_GRADING_STATUS_NOT_GRADED;
            }
        }
    }

    /**
     * The id used to uniquily identify the cache for this instance of the prog object.
     *
     * @return string
     */
...
/**
 * Portfolio caller class for mod_prog.
 *
 * @package   mod_prog
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class prog_portfolio_caller extends portfolio_module_caller_base {
    ...
    public function __construct($callbackargs) {
        ...
        $this->cm = get_coursemodule_from_id('prog', $this->cmid, 0, false, MUST_EXIST);
    }
    ...
    public function load_data() {
        ...
        if (empty($this->fileid)) {
            if (empty($this->sid) || empty($this->area)) {
                throw new portfolio_caller_exception('invalidfileandsubmissionid', 'mod_prog');
            }
    ...
    public function prepare_package() {
        ...
        if ($this->exporter->get('formatclass') == PORTFOLIO_FORMAT_LEAP2A) {
            ...
            if (count($files) > 1) {
                $baseid = 'prog' . $this->cmid . $this->area;
                $context = context_module::instance($this->cmid);
    ...
    public function check_permissions() {
        $context = context_module::instance($this->cmid);
        return has_capability('mod/prog:exportownsubmission', $context);
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'prog');
    }
