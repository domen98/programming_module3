...
/**
 * A custom renderer class that extends the plugin_renderer_base and is used by the prog module.
 *
 * @package mod_prog
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_prog_renderer extends plugin_renderer_base {
     ...
     * @return string
     */
    public function prog_files(context $context, $userid, $filearea, $component) {
        return $this->render(new prog_files($context, $userid, $filearea, $component));
    }

    /**
     * Rendering assignment files
     *
     * @param prog_files $tree
     * @return string
     */
    public function render_prog_files(prog_files $tree) {
        $this->htmlid = html_writer::random_id('prog_files_tree');
        $this->page->requires->js_init_call('M.mod_prog.init_tree', array(true, $this->htmlid));
     ...
     * @param prog_gradingmessage $result The result to render
     * @return string
     */
    public function render_prog_gradingmessage(prog_gradingmessage $result) {
     ...
     * @param prog_form $form The form to render
     * @return string
     */
    public function render_prog_form(prog_form $form) {
        $o = '';
     ...
     *
     * @param prog_user_summary $summary The user summary to render
     * @return string
     */
    public function render_prog_user_summary(prog_user_summary $summary) {
        $o = '';
        $supendedclass = '';
        $suspendedicon = '';
        ...
        if ($summary->blindmarking) {
            $o .= get_string('hiddenuser', 'prog') . $summary->uniqueidforuser.$suspendedicon;
        } else {
     ...
     *
     * @param prog_submit_for_grading_page $page
     * @return string
     */
    public function render_prog_submit_for_grading_page($page) {
        $o = '';

        $o .= $this->output->container_start('submitforgrading');
        $o .= $this->output->heading(get_string('submitassignment', 'prog'), 3);
        $o .= $this->output->spacer(array('height'=>30));

        $cancelurl = new moodle_url('/mod/prog/view.php', array('id' => $page->coursemoduleid));
        if (count($page->notifications)) {
            // At least one of the submission plugins is not ready for submission.

            $o .= $this->output->heading(get_string('submissionnotready', 'prog'), 4);
     ...
     * Render the header.
     *
     * @param prog_header $header
     * @return string
     */
    public function render_prog_header(prog_header $header) {
        $o = '';
        ...
        $this->page->set_title(get_string('pluginname', 'prog'));
        $this->page->set_heading($this->page->course->fullname);

        $o .= $this->output->header();
        $heading = format_string($header->prog->name, false, array('context' => $header->context));
        ...
        if ($header->showintro) {
            ...
            $o .= format_module_intro('prog', $header->prog, $header->coursemoduleid);
            $o .= $header->postfix;
            $o .= $this->output->box_end();
        }
     ...
     *
     * @param prog_plugin_header $header
     * @return string
     */
    public function render_prog_plugin_header(prog_plugin_header $header) {
     ...
     *
     * @param prog_grading_summary $summary
     * @return string
     */
    public function render_prog_grading_summary(prog_grading_summary $summary) {
        // Create a table for the data.
        $o = '';
        $o .= $this->output->container_start('gradingsummary');
        $o .= $this->output->heading(get_string('gradingsummary', 'prog'), 3);
        ...
        if ($summary->teamsubmission) {
            if ($summary->warnofungroupedusers) {
                $o .= $this->output->notification(get_string('ungroupedusers', 'prog'));
            }

            $this->add_table_row_tuple($t, get_string('numberofteams', 'prog'),
                                       $summary->participantcount);
        } else {
            $this->add_table_row_tuple($t, get_string('numberofparticipants', 'prog'),
                                       $summary->participantcount);
        }
        ...
        if ($summary->submissiondraftsenabled && $summary->submissionsenabled) {
            $this->add_table_row_tuple($t, get_string('numberofdraftsubmissions', 'prog'),
                                       $summary->submissiondraftscount);
        }

        // Submitted for grading.
        if ($summary->submissionsenabled) {
            $this->add_table_row_tuple($t, get_string('numberofsubmittedassignments', 'prog'),
                                       $summary->submissionssubmittedcount);
            if (!$summary->teamsubmission) {
                $this->add_table_row_tuple($t, get_string('numberofsubmissionsneedgrading', 'prog'),
        ...
        if ($summary->duedate) {
            ...
            $this->add_table_row_tuple($t, get_string('duedate', 'prog'),
                                       userdate($duedate));

            // Time remaining.
            $due = '';
            if ($duedate - $time <= 0) {
                $due = get_string('assignmentisdue', 'prog');
            } else {
                $due = format_time($duedate - $time);
            }
            $this->add_table_row_tuple($t, get_string('timeremaining', 'prog'), $due);

            if ($duedate < $time) {
                $cutoffdate = $summary->cutoffdate;
                if ($cutoffdate) {
                    if ($cutoffdate > $time) {
                        $late = get_string('latesubmissionsaccepted', 'prog', userdate($summary->cutoffdate));
                    } else {
                        $late = get_string('nomoresubmissionsaccepted', 'prog');
                    }
                    $this->add_table_row_tuple($t, get_string('latesubmissions', 'prog'), $late);
                }
            }
        ...
        $url = new moodle_url('/mod/prog/view.php', $urlparams);
        $o .= $this->output->action_link($url, get_string('viewgrading', 'prog'));
        $o .= $this->output->container_end();
     ...
     *
     * @param prog_feedback_status $status
     * @return string
     */
    public function render_prog_feedback_status(prog_feedback_status $status) {
        global $DB, $CFG;
        $o = '';

        $o .= $this->output->container_start('feedback');
        $o .= $this->output->heading(get_string('feedback', 'prog'), 3);
        ...
        if (isset($status->gradefordisplay)) {
            ...
            // Grade date.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('gradedon', 'prog'));
        ...
        if ($status->grader) {
            // Grader.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('gradedby', 'prog'));
        ...
        foreach ($status->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() &&
                ...
                $displaymode = prog_feedback_plugin_feedback::SUMMARY;
                $pluginfeedback = new prog_feedback_plugin_feedback($plugin,
                                                                      $status->grade,
                                                                      $displaymode,
     ...
     *
     * @param prog_submission_status $status
     * @return string
     */
	...
	global $CFG;
        ...
        if ($status->allowsubmissionsfromdate &&
            ...
            if ($status->alwaysshowdescription) {
                ...
                $o .= get_string('allowsubmissionsfromdatesummary', 'prog', $date);
            } else {
                $date = userdate($status->allowsubmissionsfromdate);
                $o .= get_string('allowsubmissionsanddescriptionfromdatesummary', 'prog', $date);
            }
            $o .= $this->output->box_end();
        }
        ...
        if ($status->teamsubmissionenabled) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('submissionteam', 'prog'));
            ...
            if ($group) {
                ...
                if (count($status->usergroups) == 0) {
                    $cell2 = new html_table_cell(
                        html_writer::span(get_string('noteam', 'prog'), 'alert alert-error')
                    );
                } else if (count($status->usergroups) > 1) {
                    $cell2 = new html_table_cell(
                        html_writer::span(get_string('multipleteams', 'prog'), 'alert alert-error')
                    );
                }
            } else {
                $cell2 = new html_table_cell(get_string('defaultteam', 'prog'));
            }
            $row->cells = array($cell1, $cell2);
            $t->data[] = $row;
        }

        if ($status->attemptreopenmethod != PROG_ATTEMPT_REOPEN_METHOD_NONE) {
            $currentattempt = 1;
            if (!$status->teamsubmissionenabled) {
            ...
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('attemptnumber', 'prog'));
            $maxattempts = $status->maxattempts;
            if ($maxattempts == PROG_UNLIMITED_ATTEMPTS) {
                $message = get_string('currentattempt', 'prog', $currentattempt);
            } else {
                $message = get_string('currentattemptof', 'prog', array('attemptnumber'=>$currentattempt,
        ...
        $row = new html_table_row();
        $cell1 = new html_table_cell(get_string('submissionstatus', 'prog'));
        if (!$status->teamsubmissionenabled) {
            if ($status->submission && $status->submission->status != PROG_SUBMISSION_STATUS_NEW) {
                $statusstr = get_string('submissionstatus_' . $status->submission->status, 'prog');
                $cell2 = new html_table_cell($statusstr);
                $cell2->attributes = array('class'=>'submissionstatus' . $status->submission->status);
            } else {
                if (!$status->submissionsenabled) {
                    $cell2 = new html_table_cell(get_string('noonlinesubmissions', 'prog'));
                } else {
                    $cell2 = new html_table_cell(get_string('noattempt', 'prog'));
                }
            }
            $row->cells = array($cell1, $cell2);
            $t->data[] = $row;
            ...
            if (!$group && $status->preventsubmissionnotingroup) {
                $cell2 = new html_table_cell(get_string('nosubmission', 'prog'));
            } else if ($status->teamsubmission && $status->teamsubmission->status != PROG_SUBMISSION_STATUS_NEW) {
                $teamstatus = $status->teamsubmission->status;
                $submissionsummary = get_string('submissionstatus_' . $teamstatus, 'prog');
                $groupid = 0;
                ...
                foreach ($members as $member) {
                    ...
                    $url = new moodle_url('/user/view.php', $urlparams);
                    if ($status->view == prog_submission_status::GRADER_VIEW && $status->blindmarking) {
                ...
                if (count($userslist) > 0) {
                    ...
                    $formatteduserstr = get_string('userswhoneedtosubmit', 'prog', $userstr);
        ...
        if ($status->locked) {
            ...
            $cell2 = new html_table_cell(get_string('submissionslocked', 'prog'));
        ...
        // Grading status.
        $row = new html_table_row();
        $cell1 = new html_table_cell(get_string('gradingstatus', 'prog'));

        if ($status->gradingstatus == PROG_GRADING_STATUS_GRADED ||
            $status->gradingstatus == PROG_GRADING_STATUS_NOT_GRADED) {
            $cell2 = new html_table_cell(get_string($status->gradingstatus, 'prog'));
        } else {
            $gradingstatus = 'markingworkflowstate' . $status->gradingstatus;
            $cell2 = new html_table_cell(get_string($gradingstatus, 'prog'));
        }
        if ($status->gradingstatus == PROG_GRADING_STATUS_GRADED ||
            $status->gradingstatus == PROG_MARKING_WORKFLOW_STATE_RELEASED) {
        ...
        if ($duedate > 0) {
            // Due date.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('duedate', 'prog'));
            $cell2 = new html_table_cell(userdate($duedate));
            $row->cells = array($cell1, $cell2);
            $t->data[] = $row;

            if ($status->view == prog_submission_status::GRADER_VIEW) {
                if ($status->cutoffdate) {
                    // Cut off date.
                    $row = new html_table_row();
                    $cell1 = new html_table_cell(get_string('cutoffdate', 'prog'));
            ...
            if ($status->extensionduedate) {
                // Extension date.
                $row = new html_table_row();
                $cell1 = new html_table_cell(get_string('extensionduedate', 'prog'));
            ...
            // Time remaining.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('timeremaining', 'prog'));
            if ($duedate - $time <= 0) {
                if (!$submission ||
                        $submission->status != PROG_SUBMISSION_STATUS_SUBMITTED) {
                    if ($status->submissionsenabled) {
                        $overduestr = get_string('overdue', 'prog', format_time($time - $duedate));
                        $cell2 = new html_table_cell($overduestr);
                        $cell2->attributes = array('class'=>'overdue');
                    } else {
                        $cell2 = new html_table_cell(get_string('duedatereached', 'prog'));
                    }
                } else {
        ...
        if ($status->view == prog_submission_status::GRADER_VIEW) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('editingstatus', 'prog'));
            if ($status->canedit) {
                $cell2 = new html_table_cell(get_string('submissioneditable', 'prog'));
                $cell2->attributes = array('class'=>'submissioneditable');
            } else {
                $cell2 = new html_table_cell(get_string('submissionnoteditable', 'prog'));
        ...
        if (!empty($status->gradingcontrollerpreview)) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('gradingmethodpreview', 'prog'));
        ...
        if ($submission) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('timemodified', 'prog'));
            ...
            if (!$status->teamsubmission || $status->submissiongroup != false || !$status->preventsubmissionnotingroup) {
                foreach ($status->submissionplugins as $plugin) {
                    ...
                    if ($plugin->is_enabled() &&
                        ...
                        $row = new html_table_row();
                        $cell1 = new html_table_cell($plugin->get_name());
                        $displaymode = prog_submission_plugin_submission::SUMMARY;
                        $pluginsubmission = new prog_submission_plugin_submission($plugin,
                            $submission,
                            $displaymode,
                            $status->coursemoduleid,
                            $status->returnaction,
        ...
        if ($status->view == prog_submission_status::STUDENT_VIEW) {
			...
			/*
            if ($status->canedit) {
                if (!$submission || $submission->status == PROG_SUBMISSION_STATUS_NEW) {
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid, 'action' => 'editsubmission');
                    $o .= $this->output->single_button(new moodle_url('/mod/prog/view.php', $urlparams),get_string('addsubmission', 'prog'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('editsubmission_help', 'prog');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
                }
				else if ($submission->status == PROG_SUBMISSION_STATUS_REOPENED) {
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid,
                                       'action' => 'editprevioussubmission',
                                       'sesskey'=>sesskey());
                    $o .= $this->output->single_button(new moodle_url('/mod/prog/view.php', $urlparams),
                                                       get_string('addnewattemptfromprevious', 'prog'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('addnewattemptfromprevious_help', 'prog');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid, 'action' => 'editsubmission');
                    $o .= $this->output->single_button(new moodle_url('/mod/prog/view.php', $urlparams),
                                                       get_string('addnewattempt', 'prog'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('addnewattempt_help', 'prog');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
                }
				else {
                    ...
                    $urlparams = array('id' => $status->coursemoduleid, 'action' => 'editsubmission');
                    $o .= $this->output->single_button(new moodle_url('/mod/prog/view.php', $urlparams),
                                                       get_string('editsubmission', 'prog'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('editsubmission_help', 'prog');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
			...
			*/
            if ($status->cansubmit) {
                ...
                $o .= $this->output->box_start('generalbox submissionaction');
                $o .= $this->output->single_button(new moodle_url('/mod/prog/view.php', $urlparams),
                                                   get_string('submitassignment', 'prog'), 'get');
                $o .= $this->output->box_start('boxaligncenter submithelp');
                $o .= get_string('submitassignment_help', 'prog');
                $o .= $this->output->box_end();
                $o .= $this->output->box_end();
            }
        }
     ...
     *
     * @param prog_attempt_history $history
     * @return string
     */
    public function render_prog_attempt_history(prog_attempt_history $history) {
        $o = '';

        $submittedstr = get_string('submitted', 'prog');
        $gradestr = get_string('grade');
        $gradedonstr = get_string('gradedon', 'prog');
        $gradedbystr = get_string('gradedby', 'prog');
        ...
        $o .= $this->heading(get_string('attempthistory', 'prog'), 3);
        ...
        foreach ($history->submissions as $i => $submission) {
            ...
            if ($submission) {
                ...
                $submissionsummary = get_string('nosubmission', 'prog');
            }

            $attemptsummaryparams = array('attemptnumber'=>$submission->attemptnumber+1,
                                          'submissionsummary'=>$submissionsummary);
            $o .= $this->heading(get_string('attemptheading', 'prog', $attemptsummaryparams), 4);

            $t = new html_table();

            if ($submission) {
                $cell1 = new html_table_cell(get_string('submissionstatus', 'prog'));
                $cell2 = new html_table_cell(get_string('submissionstatus_' . $submission->status, 'prog'));
                ...
                foreach ($history->submissionplugins as $plugin) {
                    ...
                    if ($plugin->is_enabled() &&
                        ...
                        $pluginsubmission = new prog_submission_plugin_submission($plugin,
                                                                                    $submission,
                                                                                    prog_submission_plugin_submission::SUMMARY,
            ...
            if ($grade) {
                // Heading 'feedback'.
                $title = get_string('feedback', 'prog', $i);
                ...
                if ($history->cangrade) {
                    ...
                    $icon = new pix_icon('gradefeedback',
                                            get_string('editattemptfeedback', 'prog', $grade->attemptnumber+1),
                                            'mod_prog');
                ...
                foreach ($history->feedbackplugins as $plugin) {
                    if ($plugin->is_enabled() &&
                        ...
                        $cell1 = new html_table_cell($plugin->get_name());
                        $pluginfeedback = new prog_feedback_plugin_feedback(
                            $plugin, $grade, prog_feedback_plugin_feedback::SUMMARY, $history->coursemoduleid,
        ...
        $this->page->requires->yui_module('moodle-mod_prog-history', 'Y.one("#' . $containerid . '").history');
     ...
     *
     * @param prog_submission_plugin_submission $submissionplugin
     * @return string
     */
    public function render_prog_submission_plugin_submission(prog_submission_plugin_submission $submissionplugin) {
        $o = '';

        if ($submissionplugin->view == prog_submission_plugin_submission::SUMMARY) {
            $showviewlink = false;
            ...
            $link = '';
            if ($showviewlink) {
                $previewstr = get_string('viewsubmission', 'prog');
                $icon = $this->output->pix_icon('t/preview', $previewstr);

                $expandstr = get_string('viewfull', 'prog');
                ...
                $this->page->requires->js_init_call('M.mod_prog.init_plugin_summary', $jsparams);
            ...
            if ($showviewlink) {
                ...
                $o .= $this->output->pix_icon('t/switch_minus',
                                              get_string('viewsummary', 'prog'),
                                              null,
                                              array('class'=>$classes));
                $o .= $submissionplugin->plugin->view($submissionplugin->submission);
                $o .= $this->output->box_end();
            }
        } else if ($submissionplugin->view == prog_submission_plugin_submission::FULL) {
     ...
     *
     * @param prog_grading_table $table
     * @return string
     */
    public function render_prog_grading_table(prog_grading_table $table) {
        $o = '';
        $o .= $this->output->box_start('boxaligncenter gradingtable');

        $this->page->requires->js_init_call('M.mod_prog.init_grading_table', array());
        $this->page->requires->string_for_js('nousersselected', 'prog');
        $this->page->requires->string_for_js('batchoperationconfirmgrantextension', 'prog');
        $this->page->requires->string_for_js('batchoperationconfirmlock', 'prog');
        $this->page->requires->string_for_js('batchoperationconfirmreverttodraft', 'prog');
        $this->page->requires->string_for_js('batchoperationconfirmunlock', 'prog');
        $this->page->requires->string_for_js('batchoperationconfirmaddattempt', 'prog');
        $this->page->requires->string_for_js('batchoperationconfirmsetmarkingworkflowstate', 'prog');
        $this->page->requires->string_for_js('batchoperationconfirmsetmarkingallocation', 'prog');
        $this->page->requires->string_for_js('editaction', 'prog');
     ...
     *
     * @param prog_feedback_plugin_feedback $feedbackplugin
     * @return string
     */
    public function render_prog_feedback_plugin_feedback(prog_feedback_plugin_feedback $feedbackplugin) {
        $o = '';

        if ($feedbackplugin->view == prog_feedback_plugin_feedback::SUMMARY) {
            $showviewlink = false;
            ...
            $link = '';
            if ($showviewlink) {
                $previewstr = get_string('viewfeedback', 'prog');
                $icon = $this->output->pix_icon('t/preview', $previewstr);
        ...
        } else if ($feedbackplugin->view == prog_feedback_plugin_feedback::FULL) {
     ...
     *
     * @param prog_course_index_summary $indexsummary
     * @return string
     */
    public function render_prog_course_index_summary(prog_course_index_summary $indexsummary) {
        $o = '';

        $strplural = get_string('modulenameplural', 'prog');
        $strsectionname  = $indexsummary->courseformatname;
        $strduedate = get_string('duedate', 'prog');
        $strsubmission = get_string('submission', 'prog');
        ...
        foreach ($indexsummary->assignments as $info) {
            ...
            $link = html_writer::link(new moodle_url('/mod/prog/view.php', $params),
                                      $info['cmname']);
     ...
     * @return string
     */
    protected function htmllize_tree(prog_files $tree, $dir) {
        global $CFG;
        $yuiconfig = array();
        $yuiconfig['type'] = 'html';
