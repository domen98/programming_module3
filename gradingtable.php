 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class prog_grading_table extends table_sql implements renderable {
    /** @var prog $assignment */
     ...
     *
     * @param prog $assignment The assignment class
     ...
     */
    public function __construct(prog $assignment,
                                $perpage,
                                $filter,
                                $rowoffset,
                                $quickgrading,
        ...
        parent::__construct('mod_prog_grading');
        $this->is_persistent(true);
        $this->assignment = $assignment;

        // Check permissions up front.
        $this->hasgrantextension = has_capability('mod/prog:grantextension',
                                                  $this->assignment->get_context());
        $this->hasgrade = $this->assignment->can_grade();

        // Check if we have the elevated view capablities to see the blind details.
        $this->hasviewblind = has_capability('mod/prog:viewblinddetails',
                $this->assignment->get_context());
        ...
        $url = new moodle_url($CFG->wwwroot . '/mod/prog/view.php', $urlparams);
        ...
        $from = '{user} u
                         LEFT JOIN {prog_submission} s
                               ...
                               AND s.latest = 1
                         LEFT JOIN {prog_grades} g
                                ON u.id = g.userid
                               AND g.assignment = :assignmentid2 ';

        // For group submissions we don't immediately create an entry in the prog_submission table for each user,
        ...
        $from .= 'LEFT JOIN {prog_user_flags} uf
                         ON u.id = uf.userid
        ...
        if ($this->assignment->is_any_submission_plugin_enabled()) {
            if ($filter == PROG_FILTER_SUBMITTED) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted) ';
                $params['submitted'] = PROG_SUBMISSION_STATUS_SUBMITTED;

            } else if ($filter == PROG_FILTER_NOT_SUBMITTED) {
                $where .= ' AND (s.timemodified IS NULL OR s.status != :submitted) ';
                $params['submitted'] = PROG_SUBMISSION_STATUS_SUBMITTED;
            } else if ($filter == PROG_FILTER_REQUIRE_GRADING) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted AND
                                 (s.timemodified >= g.timemodified OR g.timemodified IS NULL OR g.grade IS NULL))';
                $params['submitted'] = PROG_SUBMISSION_STATUS_SUBMITTED;

            } else if (strpos($filter, PROG_FILTER_SINGLE_USER) === 0) {
        ...
        if ($this->assignment->get_instance()->markingworkflow &&
            ...
            if (has_capability('mod/prog:manageallocations', $this->assignment->get_context())) {
                // Check to see if marker filter is set.
                $markerfilter = (int)get_user_preferences('prog_markerfilter', '');
                if (!empty($markerfilter)) {
                    if ($markerfilter == PROG_MARKER_FILTER_NO_MARKER) {
        ...
        if ($this->assignment->get_instance()->markingworkflow) {
            ...
            if (!empty($workflowstates)) {
                ...
                if ($workflowfilter == PROG_MARKING_WORKFLOW_STATE_NOTMARKED) {
                    $where .= ' AND (uf.workflowstate = :workflowstate OR uf.workflowstate IS NULL OR '.
                        $DB->sql_isempty('prog_user_flags', 'workflowstate', true, true).')';
        ...
        if ($this->hasviewblind || !$this->assignment->is_blind_marking()) {
            if (!$this->is_downloading()) {
                ...
                $columns[] = 'recordid';
                $headers[] = get_string('recordid', 'prog');
            }
        ...
        // Submission status.
        $columns[] = 'status';
        $headers[] = get_string('status', 'prog');

        // Team submission columns.
        if ($assignment->get_instance()->teamsubmission) {
            $columns[] = 'team';
            $headers[] = get_string('submissionteam', 'prog');
        }
        // Allocated marker.
        if ($this->assignment->get_instance()->markingworkflow &&
            $this->assignment->get_instance()->markingallocation &&
            has_capability('mod/prog:manageallocations', $this->assignment->get_context())) {
            // Add a column for the allocated marker.
            $columns[] = 'allocatedmarker';
            $headers[] = get_string('marker', 'prog');
        }
        // Grade.
        $columns[] = 'grade';
        ...
        if ($assignment->is_any_submission_plugin_enabled()) {
            ...
            $headers[] = get_string('lastmodifiedsubmission', 'prog');
        ...
        // Time marked.
        $columns[] = 'timemarked';
        $headers[] = get_string('lastmodifiedgrade', 'prog');
    ...
    public function col_recordid(stdClass $row) {
        return get_string('hiddenuser', 'prog') .
    ...
    public function col_workflowstatus(stdClass $row) {
        ...
        if (empty($workflowstate)) {
            $workflowstate = PROG_MARKING_WORKFLOW_STATE_NOTMARKED;
        }
        if ($this->quickgrading && !$gradingdisabled) {
            ...
            $o .= $this->output->container(get_string('markingworkflowstate' . $workflowstate, 'prog'), $workflowstate);
        }
        return $o;
    }
    ...
    public function col_workflowstate($row) {
        ...
        if (empty($state)) {
            $state = PROG_MARKING_WORKFLOW_STATE_NOTMARKED;
        }

        return get_string('markingworkflowstate' . $state, 'prog');
    }
    ...
    public function col_allocatedmarker(stdClass $row) {
        ...
        if ($markers === null) {
            ...
            $markers = get_users_by_capability($this->assignment->get_context(), 'mod/prog:grade', '', $sort);
            $markerlist[0] = get_string('choosemarker', 'prog');
        ...
        if ($this->quickgrading && has_capability('mod/prog:manageallocations', $this->assignment->get_context()) &&
            (empty($row->workflowstate) ||
             $row->workflowstate == PROG_MARKING_WORKFLOW_STATE_INMARKING ||
             $row->workflowstate == PROG_MARKING_WORKFLOW_STATE_NOTMARKED)) {
    ...
    public function col_team(stdClass $row) {
        ...
        if ($group) {
            ...
            if (count($usergroups) > 1) {
                return get_string('multipleteamsgrader', 'prog');
            } else {
                return get_string('noteamgrader', 'prog');
            }
        }
        return get_string('defaultteam', 'prog');
    }
    ...
    public function col_select(stdClass $row) {
        ...
        $selectcol .= get_string('selectuser', 'prog', $this->assignment->fullname($row));
        $selectcol .= '</label>';
    ...
    public function col_grade(stdClass $row) {
        ...
        if (!$this->is_downloading() && $this->hasgrade) {
            ...
            $icon = $this->output->pix_icon('gradefeedback',
                                            get_string('gradeuser', 'prog', $name),
                                            'mod_prog');
    ...
    public function col_timesubmitted(stdClass $row) {
        ...
        $submission = false;
        $this->get_group_and_submission($row->id, $group, $submission, -1);
        if ($submission && $submission->timemodified && $submission->status != PROG_SUBMISSION_STATUS_NEW) {
    ...
    public function col_status(stdClass $row) {
        ...
        if ($this->assignment->is_any_submission_plugin_enabled()) {
            ...
            $o .= $this->output->container(get_string('submissionstatus_' . $status, 'prog'),
            ...
            if ($row->locked) {
                $lockedstr = get_string('submissionslockedshort', 'prog');
            ...
            if (!$instance->markingworkflow) {
                if ($row->grade !== null && $row->grade >= 0) {
                    $o .= $this->output->container(get_string('graded', 'prog'), 'submissiongraded');
                } else if (!$timesubmitted) {
                    $now = time();
                    if ($due && ($now > $due)) {
                        $overduestr = get_string('overdue', 'prog', format_time($now - $due));
        ...
        if ($row->extensionduedate) {
            ...
            $extensionstr = get_string('userextensiondate', 'prog', $userdate);
    ...
    public function col_userid(stdClass $row) {
        ...
        if (!$row->grade) {
            ...
            $description = get_string('updategrade', 'prog');
        }
        ...
        if ($this->assignment->is_any_submission_plugin_enabled()) {
            if (!$row->status ||
                    $row->status == PROG_SUBMISSION_STATUS_DRAFT ||
                ...
                if (!$row->locked) {
                    ...
                    $description = get_string('preventsubmissionsshort', 'prog');
                    ...
                    $urlparams = array('id' => $this->assignment->get_course_module()->id,
                                       ...
                                       'page'=>$this->currpage);
                    $url = new moodle_url('/mod/prog/view.php', $urlparams);
                    $description = get_string('allowsubmissionsshort', 'prog');
            ...
            if ($submissionsopen &&
                ...
                $urlparams = array('id' => $this->assignment->get_course_module()->id,
                                   ...
                                   'page'=>$this->currpage);
                $url = new moodle_url('/mod/prog/view.php', $urlparams);
                $description = get_string('editsubmission', 'prog');
        ...
        if (($this->assignment->get_instance()->duedate ||
             ...
             $urlparams = array('id' => $this->assignment->get_course_module()->id,
                                ...
                                'page' => $this->currpage);
             $url = new moodle_url('/mod/prog/view.php', $urlparams);
             $description = get_string('grantextension', 'prog');
        ...
        if ($row->status == PROG_SUBMISSION_STATUS_SUBMITTED &&
                $this->assignment->get_instance()->submissiondrafts) {
            $urlparams = array('id' => $this->assignment->get_course_module()->id,
                               'userid'=>$row->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/prog/view.php', $urlparams);
            $description = get_string('reverttodraftshort', 'prog');
        ...
        if ($row->status == PROG_SUBMISSION_STATUS_DRAFT &&
                $this->assignment->get_instance()->submissiondrafts &&
                $caneditsubmission &&
                $submissionsopen &&
                $row->id != $USER->id) {
            $urlparams = array('id' => $this->assignment->get_course_module()->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/prog/view.php', $urlparams);
            $description = get_string('submitforgrading', 'prog');
        ...
        $ismanual = $this->assignment->get_instance()->attemptreopenmethod == PROG_ATTEMPT_REOPEN_METHOD_MANUAL;
        $hassubmission = !empty($row->status);
        $notreopened = $hassubmission && $row->status != PROG_SUBMISSION_STATUS_REOPENED;
        $isunlimited = $this->assignment->get_instance()->maxattempts == PROG_UNLIMITED_ATTEMPTS;
        ...
        if ($ismanual && $hassubmission && $notreopened && $hasattempts) {
            $urlparams = array('id' => $this->assignment->get_course_module()->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/prog/view.php', $urlparams);
            $description = get_string('addattempt', 'prog');
     ...
     *
     * @param prog_plugin $plugin Submission plugin or feedback plugin
     ...
     */
    private function format_plugin_summary_with_link(prog_plugin $plugin,
                                                     stdClass $item,
                                                     $returnaction,
                                                     $returnparams) {
        $link = '';
        ...
        $separator = '';
        if ($showviewlink) {
            $viewstr = get_string('view' . substr($plugin->get_subtype(), strlen('prog')), 'prog');
    ...
    public function other_cols($colname, $row) {
        ...
        if ($plugin->is_visible() && $plugin->is_enabled()) {
            if ($plugin->get_subtype() == 'progsubmission') {
                if ($this->assignment->get_instance()->teamsubmission) {
                    $group = false;
                    $submission = false;
                    ...
                    if ($row->status == PROG_SUBMISSION_STATUS_REOPENED) {
                        // For a newly reopened submission - we want to show the previous submission in the table.
