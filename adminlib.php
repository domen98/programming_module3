...
/**
 * This file contains the classes for the admin settings of the prog module.
 *
 * @package   mod_prog
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class prog_admin_page_manage_prog_plugins extends admin_externalpage {
    ...
    public function __construct($subtype) {
        ...
        $url = new moodle_url('/mod/prog/adminmanageplugins.php', array('subtype'=>$subtype));
        parent::__construct('manage' . $subtype . 'plugins',
                            get_string('manage' . $subtype . 'plugins', 'prog'),
                            $url);
    }
...
class prog_plugin_manager {
     ...
     * @param string $subtype - either progsubmission or progfeedback
     */
    public function __construct($subtype) {
        $this->pageurl = new moodle_url('/mod/prog/adminmanageplugins.php', array('subtype'=>$subtype));
        $this->subtype = $subtype;
    }
    ...
    private function view_plugins_table() {
        ...
        $table->define_columns(array('pluginname', 'version', 'hideshow', 'order',
                'settings', 'uninstall'));
        $table->define_headers(array(get_string($this->subtype . 'pluginname', 'prog'),
                get_string('version'), get_string('hideshow', 'prog'),
        ...
        $shortsubtype = substr($this->subtype, strlen('prog'));
        ...
        foreach ($plugins as $idx => $plugin) {
            ...
            $exists = file_exists($CFG->dirroot . '/mod/prog/' . $shortsubtype . '/' . $plugin . '/settings.php');
            if ($row[1] != '' && $exists) {
    ...
    private function view_header() {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('manage' . $this->subtype . 'plugins', 'prog'));
    }
