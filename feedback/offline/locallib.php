 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class prog_feedback_offline extends prog_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'progfeedback_offline');
    }
    ...
    public function process_import_grades($draftid, $importid, $ignoremodified) {
        ...
        $gradeimporter = new progfeedback_offline_grade_importer($importid, $this->assignment);
        ...
        if (!$gradeimporter->init()) {
            $thisurl = new moodle_url('/mod/prog/view.php', array('action'=>'viewpluginpage',
                                                                     'pluginsubtype'=>'progfeedback',
                                                                     'plugin'=>'offline',
                                                                     'pluginaction'=>'uploadgrades',
                                                                     'id'=>$assignment->get_course_module()->id));
            print_error('invalidgradeimport', 'progfeedback_offline', $thisurl);
            return;
        }
        ...
        $o .= $renderer->render(new prog_header($this->assignment->get_instance(),
                                                  $this->assignment->get_context(),
                                                  false,
                                                  $this->assignment->get_course_module()->id,
                                                  get_string('importgrades', 'progfeedback_offline')));
        $o .= $renderer->box(get_string('updatedgrades', 'progfeedback_offline', $updatecount));
    ...
    public function upload_grades() {
        ...
        require_capability('mod/prog:grade', $this->assignment->get_context());
        require_once($CFG->dirroot . '/mod/prog/feedback/offline/uploadgradesform.php');
        require_once($CFG->dirroot . '/mod/prog/feedback/offline/importgradesform.php');
        require_once($CFG->dirroot . '/mod/prog/feedback/offline/importgradeslib.php');
        require_once($CFG->libdir . '/csvlib.class.php');

        $mform = new progfeedback_offline_upload_grades_form(null,
        ...
        if ($mform->is_cancelled()) {
            ...
            $importid = csv_import_reader::get_new_iid('progfeedback_offline');
            $gradeimporter = new progfeedback_offline_grade_importer($importid, $this->assignment);
            ...
            $mform = new progfeedback_offline_import_grades_form(null, array('assignment'=>$this->assignment,
                                                                       'csvdata'=>$csvdata,
            ...
            $o .= $renderer->render(new prog_header($this->assignment->get_instance(),
                                                            ...
                                                            false,
                                                            $this->assignment->get_course_module()->id,
                                                            get_string('confirmimport', 'progfeedback_offline')));
            $o .= $renderer->render(new prog_form('confirmimport', $mform));
            $o .= $renderer->render_footer();
        } else if ($confirm) {
            ...
            $o .= $renderer->render(new prog_header($this->assignment->get_instance(),
                                                            ...
                                                            false,
                                                            $this->assignment->get_course_module()->id,
                                                            get_string('uploadgrades', 'progfeedback_offline')));
            $o .= $renderer->render(new prog_form('batchuploadfiles', $mform));
            $o .= $renderer->render_footer();
        }
    ...
    public function download_grades() {
        ...
        $filename = clean_filename(get_string('offlinegradingworksheet', 'progfeedback_offline') . '-' .
                                   $this->assignment->get_course()->shortname . '-' .
                                   $this->assignment->get_instance()->name . '-' .
                                   $groupname .
                                   $this->assignment->get_course_module()->id);

        $table = new prog_grading_table($this->assignment, 0, '', 0, false, $filename);
    ...
    public function get_grading_actions() {
        return array('uploadgrades'=>get_string('uploadgrades', 'progfeedback_offline'),
                    'downloadgrades'=>get_string('downloadgrades', 'progfeedback_offline'));
    }
    ...
    public function is_enabled() {
        if ($this->enabledcache === null) {
            $gradingmanager = get_grading_manager($this->assignment->get_context(), 'mod_prog', 'submissions');
