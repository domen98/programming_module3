 ...
 * to restore one prog_feedback subplugin.
 *
 * @package   progfeedback_editpdf
 * @copyright 2013 Damyon Wiese
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_progfeedback_editpdf_subplugin extends restore_subplugin {
     ...
     * @param mixed $data
     */
    public function process_progfeedback_editpdf_files($data) {
        $data = (object)$data;

        // In this case the id is the old gradeid which will be mapped.
        $this->add_related_files('progfeedback_editpdf',
            \progfeedback_editpdf\document_services::FINAL_PDF_FILEAREA, 'grade', null, $data->gradeid);
        $this->add_related_files('progfeedback_editpdf',
            \progfeedback_editpdf\document_services::PAGE_IMAGE_READONLY_FILEAREA, 'grade', null, $data->gradeid);
        $this->add_related_files('progfeedback_editpdf', 'stamps', 'grade', null, $data->gradeid);
    }
     ...
     * @param mixed $data
     */
    public function process_progfeedback_editpdf_annotation($data) {
        global $DB;

        $data = (object)$data;
        $oldgradeid = $data->gradeid;
        // The mapping is set in the restore for the core prog activity
        // when a grade node is processed.
        $data->gradeid = $this->get_mappingid('grade', $data->gradeid);

        $DB->insert_record('progfeedback_editpdf_annot', $data);
     ...
     * @param mixed $data
     */
    public function process_progfeedback_editpdf_comment($data) {
        global $DB;
        ...
        $DB->insert_record('progfeedback_editpdf_cmnt', $data);
