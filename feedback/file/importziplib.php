 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class progfeedback_file_zip_importer {

    /**
     * Is this filename valid (contains a unique participant ID) for import?
     *
     * @param prog $assignment - The assignment instance
     ...
     * @param prog_plugin $plugin - Set to the plugin that exported the file
     ...
     * @param prog_plugin $plugin The matching plugin from the filename
    ...
    public function is_file_modified($assignment, $user, $plugin, $filename, $fileinfo) {
        ...
        } else if ($plugin->get_subtype() == 'progfeedback') {
    ...
    public function extract_files_from_zip($zipfile, $contextid) {
        ...
        core_php_time_limit::raise(PROGFEEDBACK_FILE_MAXFILEUNZIPTIME);
     ...
     *
     * @param prog $assignment - The assignment instance
     * @param prog_feedback_file $fileplugin - The file feedback plugin
    ...
    public function import_zip_files($assignment, $fileplugin) {
            ...
            if ($this->is_valid_filename_for_import($assignment, $unzippedfile, $participants, $user, $plugin, $filename)) {
                if ($this->is_file_modified($assignment, $user, $plugin, $filename, $unzippedfile)) {
                    ...
                    if ($oldfile = $fs->get_file($contextid,
                        ...
                        $newfilerecord->component = 'progfeedback_file';
                        $newfilerecord->filearea = PROGFEEDBACK_FILE_FILEAREA;
        ...
        require_once($CFG->dirroot . '/mod/prog/feedback/file/renderable.php');
        $importsummary = new progfeedback_file_import_summary($assignment->get_course_module()->id,
                                                            count($userswithnewfeedback),
                                                            $feedbackfilesadded,
                                                            $feedbackfilesupdated);

        $progrenderer = $assignment->get_renderer();
        $renderer = $PAGE->get_renderer('progfeedback_file');

        $o = '';

        $o .= $progrenderer->render(new prog_header($assignment->get_instance(),
                                                        $assignment->get_context(),
                                                        false,
                                                        $assignment->get_course_module()->id,
                                                        get_string('uploadzipsummary', 'progfeedback_file')));

        $o .= $renderer->render($importsummary);

        $o .= $progrenderer->render_footer();
        return $o;
    }
