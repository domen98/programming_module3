 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class prog_feedback_comments extends prog_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'progfeedback_comments');
    }
    ...
    public function get_feedback_comments($gradeid) {
        global $DB;
        return $DB->get_record('progfeedback_comments', array('grade'=>$gradeid));
    }
    ...
    public function get_quickgrading_html($userid, $grade) {
        ...
        $pluginname = get_string('pluginname', 'progfeedback_comments');
    ...
    public function get_editor_fields() {
        return array('comments' => get_string('pluginname', 'progfeedback_comments'));
    }
    ...
    public function set_editor_text($name, $value, $gradeid) {
        ...
        if ($name == 'comments') {
            ...
            if ($feedbackcomment) {
                ...
                return $DB->update_record('progfeedback_comments', $feedbackcomment);
            } else {
                ...
                return $DB->insert_record('progfeedback_comments', $feedbackcomment) > 0;
            }
        }
    ...
    public function save_settings(stdClass $data) {
        $this->set_config('commentinline', !empty($data->progfeedback_comments_commentinline));
        return true;
    }
    ...
    public function get_settings(MoodleQuickForm $mform) {
        ...
        if ($default === false) {
            ...
            $default = get_config('progfeedback_comments', 'inline');
        }
        $mform->addElement('selectyesno',
                           'progfeedback_comments_commentinline',
                           get_string('commentinline', 'progfeedback_comments'));
        $mform->addHelpButton('progfeedback_comments_commentinline', 'commentinline', 'progfeedback_comments');
        $mform->setDefault('progfeedback_comments_commentinline', $default);
        // Disable comment online if comment feedback plugin is disabled.
        $mform->disabledIf('progfeedback_comments_commentinline', 'progfeedback_comments_enabled', 'notchecked');
   }
    ...
    protected function convert_submission_text_to_feedback($submission, $data) {
        ...
        $data->progfeedbackcomments_editor['text'] = $text;
        $data->progfeedbackcomments_editor['format'] = $format;
    ...
    public function get_form_elements_for_user($grade, MoodleQuickForm $mform, stdClass $data, $userid) {
        ...
        if ($feedbackcomments && !empty($feedbackcomments->commenttext)) {
            $data->progfeedbackcomments_editor['text'] = $feedbackcomments->commenttext;
            $data->progfeedbackcomments_editor['format'] = $feedbackcomments->commentformat;
        } else {
        ...
        $mform->addElement('editor', 'progfeedbackcomments_editor', $this->get_name(), null, null);
    ...
    public function save(stdClass $grade, stdClass $data) {
        ...
        $feedbackcomment = $this->get_feedback_comments($grade->id);
        if ($feedbackcomment) {
            $feedbackcomment->commenttext = $data->progfeedbackcomments_editor['text'];
            $feedbackcomment->commentformat = $data->progfeedbackcomments_editor['format'];
            return $DB->update_record('progfeedback_comments', $feedbackcomment);
        } else {
            $feedbackcomment = new stdClass();
            $feedbackcomment->commenttext = $data->progfeedbackcomments_editor['text'];
    ...
    public function upgrade(context $oldcontext,
        ...
        if (!$DB->insert_record('progfeedback_comments', $feedbackcomments) > 0) {
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('progfeedback_comments',
    ...
    public function get_external_parameters() {
        ...
        return array('progfeedbackcomments_editor' => $editorstructure);
    }
